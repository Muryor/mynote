# LaTeX 试卷流水线测试与开发指南 (v3.9)

> **文档版本**：v3.9（2025-11-27）
> **最新更新（v3.9）**：
> - 🎉 **ocr_to_examx.py 两批12项修复**（测试覆盖率100%）：
>   - **第一批**：边界处理、中文分离、数学符号边界、反向定界符、冒号分离、最终清理
>   - **第二批**：集合定义修复、OCR特有错误、数学函数标准化、标点处理增强、图片属性清理
>   - 新增函数：fix_broken_set_definitions、fix_ocr_specific_errors等
>   - 测试套件：test_ocr_fixes.py（16测试）+ test_ocr_fixes_new.py（14测试）
> 
> **v1.9.1 特性**：
> - 🆕 **ocr_to_examx.py 关键修复**：
>   - 双美元符号合并不丢失内容（P0-001a）
>   - 虚数单位保持 `\text{i}` 格式（与范本一致）
>   - 图片属性清理增强（支持科学计数法）
>   - 装饰性图片检测增强（e-2/e-3 格式）
> - 🆕 **validate_tex.py 功能增强**：
>   - `\text{i}` 检查改为可选（默认不警告）
>   - 答案格式检查（单选/多选题）
>   - 难度值范围检查（0-1）
>   - 选项数量检查（choices 环境）
>   - 中文标点检测改进（栈算法）
>   - 命令行参数支持（--strict/--warn-text-i/--quiet）
