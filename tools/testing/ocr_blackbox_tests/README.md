# OCR to ExamX 黑箱测试框架

本目录包含对 `tools/core/ocr_to_examx.py` 脚本的系统性黑箱测试框架。

## 目录结构

```
ocr_blackbox_tests/
├── README.md              # 本文件
├── test_cases.md          # 测试用例定义（T001-T020）
├── run_tests.py           # 自动化测试脚本
├── analyze_results.py     # 结果分析脚本
├── SUMMARY.md             # 测试总结报告
├── ISSUES.md              # 问题清单
├── input/                 # 测试输入文件（可选）
├── output/                # 转换输出的 TeX 文件
├── reports/               # JSON 格式的测试报告
├── expected/              # 期望结果（可选）
└── edge_cases/            # 边界测试用例
```

## 快速开始

### 1. 测试单个文件

```bash
python3 tools/testing/ocr_blackbox_tests/run_tests.py \
    word_to_tex/output/gaokao_2025_national_1_preprocessed.md
```

### 2. 批量测试所有文件

```bash
for f in word_to_tex/output/*_preprocessed.md; do
    echo "========== 测试: $(basename "$f") =========="
    python3 tools/testing/ocr_blackbox_tests/run_tests.py "$f"
done
```

### 3. 生成汇总报告

```bash
python3 tools/testing/ocr_blackbox_tests/analyze_results.py
```

这将生成：
- `SUMMARY.md`: 测试总结，包含通过率统计
- `ISSUES.md`: 详细问题清单，按严重程度排序

## 测试用例

框架包含 18 个测试用例，覆盖以下方面：

### 元信息解析（T001-T005）
- ✅ T001: 【答案】提取
- ✅ T002: 【难度】提取
- ✅ T003: 【知识点】/【考点】合并
- ✅ T004: 【分析】过滤
- ✅ T005: 【详解】保留

### 数学公式（T006-T010）
- ❌ T008: 定界符平衡 **[P0 问题]**
- ✅ T009: 反向定界符检测
- ✅ T010: 双重包裹检测

### 结构完整性（T011-T013）
- ✅ T011: question 环境闭合
- ✅ T012: choices 环境检查
- ✅ T013: 题干存在性

### 图片处理（T014-T015）
- ✅ T014: IMAGE_TODO 格式
- ✅ T015: 图片属性清理

### 特殊处理（T016-T020）
- ⚠️ T016: LaTeX 转义 **[P2 问题]**
- ❌ T017: 数学模式内中文标点 **[P2 问题]**
- ✅ T018: array/cases 左括号
- ✅ T019: tabular 列格式
- ✅ T020: explain 空行

## 测试结果摘要

**最新测试**: 2025-11-28  
**测试试卷**: 9 份  
**整体通过率**: 85.2% (138/162)

### 关键问题

| 问题 | 严重程度 | 影响范围 | 状态 |
|------|---------|---------|------|
| 定界符不平衡 | P0 | 9/9 试卷 | ❌ 需要修复 |
| 数学模式内中文标点 | P2 | 9/9 试卷 | ⚠️ 建议优化 |
| LaTeX 字符未转义 | P2 | 6/9 试卷 | ⚠️ 建议优化 |

详细信息请查看 `ISSUES.md`。

## 测试报告格式

### 终端输出
```
============================================================
📋 OCR 黑箱测试报告
   文件: word_to_tex/output/exam_preprocessed.md
   时间: 2025-11-28T00:01:13.675204
============================================================

✅ [T001] 【答案】提取
   找到 19 个 \answer，Markdown 中有 19 个【答案】
   └─ 提取率: 100.0%

❌ [T008] 定界符平衡
   \( = 690, \) = 687, diff = 3
   └─ 不平衡，差值 3

------------------------------------------------------------
📊 汇总: 通过 15/18, 失败 3
============================================================
```

### JSON 报告
每个测试生成 `reports/<exam_name>_test_report.json`：

```json
{
  "exam_file": "path/to/exam.md",
  "timestamp": "2025-11-28T00:01:13.675204",
  "summary": {
    "total": 18,
    "passed": 15,
    "failed": 3
  },
  "results": [
    {
      "test_id": "T001",
      "name": "【答案】提取",
      "passed": true,
      "message": "找到 19 个 \\answer，Markdown 中有 19 个【答案】",
      "details": "提取率: 100.0%"
    }
  ]
}
```

## 添加新测试用例

1. **定义测试用例**: 在 `test_cases.md` 中添加描述
2. **实现测试方法**: 在 `run_tests.py` 中添加 `test_TXXX_<name>()` 方法
3. **更新文档**: 更新本 README 和 `test_cases.md`

示例：
```python
def test_T021_new_test(self):
    """T021: 新测试的描述"""
    # 测试逻辑
    pattern = r'...'
    matches = re.findall(pattern, self.tex_content)
    
    passed = len(matches) == 0
    self.report.add(TestResult(
        test_id="T021",
        name="新测试",
        passed=passed,
        message=f"发现 {len(matches)} 处问题"
    ))
```

## 边界测试

创建特殊的边界用例：

```bash
cat > tools/testing/ocr_blackbox_tests/edge_cases/edge_test_01.md << 'EOF'
# 边界测试 1：嵌套数学公式

1. 已知 $a = \(b + c\)$ 求解。

【答案】A
【难度】0.5
【详解】这是详解。
EOF

python3 tools/testing/ocr_blackbox_tests/run_tests.py \
    tools/testing/ocr_blackbox_tests/edge_cases/edge_test_01.md
```

## 问题严重程度分级

- **P0**: 导致编译失败（定界符不平衡、环境不闭合）
- **P1**: 导致内容错误（【答案】丢失、【分析】未过滤）
- **P2**: 导致格式问题（空行、图片属性残留、中文标点）

## CI/CD 集成

可以将测试集成到持续集成流程：

```bash
#!/bin/bash
# test.sh

# 运行所有测试
for f in word_to_tex/output/*_preprocessed.md; do
    python3 tools/testing/ocr_blackbox_tests/run_tests.py "$f" || exit 1
done

# 生成报告
python3 tools/testing/ocr_blackbox_tests/analyze_results.py

echo "✅ 所有测试通过"
```

## 常见问题

### Q: 如何只运行特定的测试用例？
A: 修改 `run_tests.py` 中的 `run_all_tests()` 方法，注释掉不需要的测试。

### Q: 测试失败时如何调试？
A: 查看 `output/` 目录下生成的 TeX 文件，对比原始 Markdown 文件。

### Q: 如何更新测试数据？
A: 将新的 `*_preprocessed.md` 文件放入 `word_to_tex/output/` 目录，重新运行测试。

## 相关工具

- `tools/scripts/validate_tex.py`: 验证生成的 TeX 文件语法
- `tools/testing/math_sm_comparison.py`: 对比数学公式状态机前后效果

## 贡献指南

发现新问题时：
1. 在 `ISSUES.md` 中记录问题
2. 添加对应的测试用例
3. 提供最小可复现示例

## 许可证

与主项目相同。
