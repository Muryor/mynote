...
\begin{question}
已知命题\emph{p}：\(\forall x \in R\)，
\(|x + 1| > 1\)；
命题\emph{q}：\(\exists x > 0\)，\(x^{3} = x\)，则(    )
\begin{choices}
  \item \emph{p}和\emph{q}都是真命题
  \item \(\neg p\)和\emph{q}都是真命题
  \item \emph{p}和\(\neg q\)都是真命题
  \item \(\neg p\)和\(\neg q\)都是真命题
\end{choices}
...
\begin{question}
已知曲线\emph{C}：\(x^{2} + y^{2} = 16)(y > 0\))，
从\emph{C}上任意一点\emph{P}向\emph{x}轴作垂线段\(PP'\)，
\(P'\)为垂足，
则线段\(PP'\)的中点\emph{M}的轨迹方程为(    )
\begin{choices}
...
注意到\(F(x),G(x)\)均为偶函数，可知该交点只能在\emph{y}轴上，\par
可得\(F(0) = G(0)\)，即\(a - 1 = 1\)，
解得\(a = 2\)，\par
若\(a = 2\)，令\(F(x) = G(x)\)，
...
\begin{center}
% IMAGE_TODO_START id=auto_9f69b9-Q7-img1 path=/Users/muryor/code/mynote/word\\_to\\_tex/output/figures/auto\\_9f69b9/media/image2.png width=60% inline=false question_index=7 sub_index=1
% CONTEXT_BEFORE: ，分别过$$A_{1},D_{1}$$作底面垂线，垂足为$$M,N$$，设$$AM = x$$，
% CONTEXT_AFTER: 则$$AA_{1} = + A_{1}M^{2}} = \sqrt{x
\begin{tikzpicture}[scale=1.05,>=Stealth,line cap=round,line join=round]
  % TODO: AI_AGENT_REPLACE_ME (id=auto_9f69b9-Q7-img1)
\end{tikzpicture}
% IMAGE_TODO_END id=auto_9f69b9-Q7-img
1
\end{center}


\begin{center}
% IMAGE_TODO_START id=auto_9f69b9-Q7-img2 path=/Users/muryor/code/mynote/word\\_to\\_tex/output/figures/auto\\_9f69b9/media/image3.png width=60% inline=false question_index=7 sub_index=1
% CONTEXT_BEFORE: 将正三棱台$$ABC - A_{1}B_{1}C_{1}$$补成正三棱锥$$P - ABC$$，
% CONTEXT_AFTER: 则$$A_{1}A$$与平面*ABC*所成角即为$$PA$$与平面*ABC*所成角， 因为$$
\begin{tikzpicture}[scale=1.05,>=Stealth,line cap=round,line join=round]
  % TODO: AI_AGENT_REPLACE_ME (id=auto_9f69b9-Q7-img2)
\end{tikzpicture}
% IMAGE_TODO_END id=auto_9f69b9-Q7-img
2
\end{center}
...
可知\(S_{\bigtriangleup ABC} = \frac{1}{2} \times 6 \times 6 \times \frac{\sqrt{3}}{2} = 9\sqrt{3},S_{\bigtriangleup A_{1}B_{1}C_{1}} = \frac{1}{2} \times 2 \times \sqrt{3} = \sqrt{3}\)，\par
...
\begin{question}
抛物线\emph{C}：\(y^{2} = 4x\)的准线为\emph{l}，
\emph{P}为\emph{C}上的动点，
过\emph{P}作\(\odot A:x^{2} + {(y - 4)}^{2} = 1\)的一条切线，
\emph{Q}为切点，过\emph{P}作\emph{l}的垂线，垂足为\emph{B}，
则(    )
\begin{choices}
  \item \emph{l}与\(\odot A\)相切
  \item 当\emph{P}，\emph{A}，\emph{B}三点共线时，\(|PQ| = \sqrt{15}\)
  \item 当\(|PB| = 2\)时，\(PA\bot AB\)
  \item 满足\(|PA| = |PB|\)的点\(P\)有且仅有2个
\end{choices}

\begin{center}
% IMAGE_TODO_START id=auto_9f69b9-Q10-img1 path=/Users/muryor/code/mynote/word\\_to\\_tex/output/figures/auto\\_9f69b9/media/image4.png width=60% inline=false question_index=10 sub_index=1
% CONTEXT_BEFORE: ，则关于$$t$$的方程有两个解， 即存在两个这样的$$P$$点，D选项正确. 
\begin{tikzpicture}[scale=1.05,>=Stealth,line cap=round,line join=round]
  % TODO: AI_AGENT_REPLACE_ME (id=auto_9f69b9-Q10-img1)
\end{tikzpicture}
% IMAGE_TODO_END id=auto_9f69b9-Q10-img
1
\end{center}
...
\begin{choices}
  ...
  \item 存在\emph{a}，\emph{b}，使得\(x = b\)为曲线\(y = f(x)\)的对称轴
  \item 存在\emph{a}，使得点\(\left( 1,f(1) \right)\)为曲线\(y = f(x)\)的对称中心
\end{choices}
...
\begin{question}
记\(S_{n}\)为等差数列\(\{ a_{n}\}\)的前\emph{n}项和，
若\(a_{3} + a_{4} = 7\)，
...
\difficulty{0.85}
\answer{95}
\explain{因为数列\(a_{n}\)为等差数列，则由题意得\(\{ \begin{array}{r}
a_{1} + 2d + a_{1} + 3d = 7 \\
3\left( a_{1} + d \right\)，解得\(\left\{ \begin{array}{r}
a_{1} = - 4 \\
d = 3
\end{array} \right.\)，\par
...
\begin{center}
% IMAGE_TODO_START id=auto_9f69b9-Q14-img1 path=/Users/muryor/code/mynote/word\\_to\\_tex/output/figures/auto\\_9f69b9/media/image5.png width=60% inline=false question_index=14 sub_index=1
% CONTEXT_BEFORE: 一个方格被选中，则共有 种选法，在所有符合上述要求的选法中，选中方格中的4个数之和的最大值是 ．
% CONTEXT_AFTER: 【答案】 24 112 【难度】0.4 【知识点】全排列问题、写出样本空间 【分析】由题意
\begin{tikzpicture}[scale=1.05,>=Stealth,line cap=round,line join=round]
  % TODO: AI_AGENT_REPLACE_ME (id=auto_9f69b9-Q14-img1)
\end{tikzpicture}
% IMAGE_TODO_END id=auto_9f69b9-Q14-img
1
\end{center}
...
\begin{question}
记\(\bigtriangleup ABC\)的内角\emph{A}，\emph{B}，
\emph{C}的对边分别为\emph{a}，\emph{b}，\emph{c}，
已知\(\sin A + \sqrt{3}\cos A = 2\)．
\begin{enumerate}[label=(\arabic*)]
  \item 求\emph{A}．
  \item 若\(a = 2\)，
\item \(\sqrt{2}b\sin C = c\sin 2B\)，
\item 求\(\bigtriangleup ABC\)的周长．
\end{enumerate}
\topics{辅助角公式；正弦定理解三角形；正弦定理边角互化的应用}
\difficulty{0.65}
\answer{(1)\(A = \frac{\pi}{6}\)
(2)\(2 + \sqrt{6} + 3\sqrt{2}\)}
\explain{(1)**方法一：常规方法(辅助角公式)**\par
...
故\(\bigtriangleup ABC\)的周长为\(2 + \sqrt{6} + 3\sqrt{2}\)}
\end{question}
...
\begin{enumerate}[label=(\arabic*)]
  \item 当\(a = 1\)时，
\item 求曲线\(y = f(x)\)在点\(\left( 1,f
  \item \right)\)处的切线方程；
  \item 若\(f(x)\)有极小值，且极小值小于0，求\emph{a}的取值范围．
\end{enumerate}
...
\(AD = 5\sqrt{3}\)，
\(\angle ADC = 90^{{^\circ}}\)，
\(\angle BAD = 30^{{^\circ}}\)，点\emph{E}，
\emph{F}满足\(\overrightarrow{AE} = \frac{2}{5}\overrightarrow{AD}\)，
\(\overrightarrow{AF} = \frac{1}{2}\overrightarrow{AB}\)，
将\(\bigtriangleup AEF\)沿*EF*翻折至\(\bigtriangleup PEF\)，
使得\(PC = 4\sqrt{3}\)．
\begin{enumerate}[label=(\arabic*)]
  \item 证明：\(EF\bot PD\)；
...
\begin{center}
% IMAGE_TODO_START id=auto_9f69b9-Q17-img1 path=/Users/muryor/code/mynote/word\\_to\\_tex/output/figures/auto\\_9f69b9/media/image6.png width=60% inline=false question_index=17 sub_index=1
% CONTEXT_BEFORE: *翻折至$$\bigtriangleup PEF$$，使得$$PC = 4$$．
% CONTEXT_AFTER: (1)证明：$$EF\bot PD$$； (2)求平面*PCD*与平面*PBF*所成的二面角的
\begin{tikzpicture}[scale=1.05,>=Stealth,line cap=round,line join=round]
  % TODO: AI_AGENT_REPLACE_ME (id=auto_9f69b9-Q17-img1)
\end{tikzpicture}
% IMAGE_TODO_END id=auto_9f69b9-Q17-img
1
\end{center}


\begin{center}
% IMAGE_TODO_START id=auto_9f69b9-Q17-img2 path=/Users/muryor/code/mynote/word\\_to\\_tex/output/figures/auto\\_9f69b9/media/image7.png width=60% inline=false question_index=17 sub_index=1
\begin{tikzpicture}[scale=1.05,>=Stealth,line cap=round,line join=round]
  % TODO: AI_AGENT_REPLACE_ME (id=auto_9f69b9-Q17-img2)
\end{tikzpicture}
% IMAGE_TODO_END id=auto_9f69b9-Q17-img
2
\end{center}
...
设甲每次投中的概率为\emph{p}，乙每次投中的概率为\emph{q}，
各次投中与否相互独立．
\begin{enumerate}[label=(\arabic*)]
  \item 若\(p = 0.4\)，\(q = 0.5\)，甲参加第一阶段比赛，
\item 求甲、乙所在队的比赛成绩不少于5分的概率．
  \item 假设\(0 < p < q\)，

\item (i)为使得甲、乙所在队的比赛成绩为15分的概率最大，应该由谁参加第一阶段比赛？

\item (ii)为使得甲、乙所在队的比赛成绩的数学期望最大，应该由谁参加第一阶段比赛？
\end{enumerate}
...
\begin{enumerate}[label=(\arabic*)]
  ...
  \item 设\(S_{n}\)为\(\bigtriangleup P_{n}P_{n + 1}P_{n + 2}\)的面积，
\item 证明：对任意正整数\(n\)，\(S_{n} = S_{n + 1}\).
\end{enumerate}

\begin{center}
% IMAGE_TODO_START id=auto_9f69b9-Q19-img1 path=/Users/muryor/code/mynote/word\\_to\\_tex/output/figures/auto\\_9f69b9/media/image8.png width=60% inline=false question_index=19 sub_index=1
% CONTEXT_AFTER: 由已知有$$m = 5^{2} - 4^{2} = 9$$，故$$C$$的方程为$$x^{2}
\begin{tikzpicture}[scale=1.05,>=Stealth,line cap=round,line join=round]
  % TODO: AI_AGENT_REPLACE_ME (id=auto_9f69b9-Q19-img1)
\end{tikzpicture}
% IMAGE_TODO_END id=auto_9f69b9-Q19-img
1
\end{center}
...
(3)方法一：先证明一个结论：对平面上三个点\(U,V,W\)，若\(\overrightarrow{UV} = (a,b)\)，\(\overrightarrow{UW} = (c,d)\)，则\(S_{\bigtriangleup UVW} = \frac{1}{2}|ad - bc|\).(若\(U,V,W\)在同一条直线上，约定\(S_{\bigtriangleup UVW} = 0\))\par
证明：\(S_{\bigtriangleup UVW} = \frac{1}{2}\left| \overrightarrow{UV} \right| \cdot \left| \overrightarrow{UW} \right|\sin\overrightarrow{UV},\overrightarrow{UW} = \frac{1}{2}\left| \overrightarrow{UV} \right| \cdot \left| \overrightarrow{UW} \right|\sqrt{1 - \cos^{2}\overrightarrow{UV},\overrightarrow{UW}}= \frac{1}{2}\left| \overrightarrow{UV} \right| \cdot \left| \overrightarrow{UW} \right|\sqrt{1 - \left( \frac{\overrightarrow{UV} \cdot \overrightarrow{UW}}{\left| \overrightarrow{UV} \right| \cdot \left| \overrightarrow{UW} \right|} \right)^{2}} = \frac{1}{2}\sqrt{\left| \overrightarrow{UV} \right|^{2} \cdot \left| \overrightarrow{UW} \right|^{2} - \left( \overrightarrow{UV} \cdot \overrightarrow{UW} \right)^{2}}= \frac{1}{2}\sqrt{\left( a^{2} + b^{2} \right)\left( c^{2} + d^{2} \right) - (ac + bd)^{2}}= \frac{1}{2}\sqrt{a^{2}c^{2} + a^{2}d^{2} + b^{2}c^{2} + b^{2}d^{2} - a^{2}c^{2} - b^{2}d^{2} - 2abcd}= \frac{1}{2}\sqrt{a^{2}d^{2} + b^{2}c^{2} - 2abcd} = \frac{1}{2}\sqrt{(ad - bc)^{2}} = \frac{1}{2}|ad - bc|\).\par
证毕，回到原题.\par
...
故利用前面已经证明的结论即得\par
\(S_{n} = S_{\bigtriangleup P_{n}P_{n + 1}P_{n + 2}} = \frac{1}{2}\left| - \left( x_{n + 1} - x_{n} \right)\left( y_{n + 2} - y_{n + 1} \right) + \left( y_{n + 1} - y_{n} \right)\left( x_{n + 2} - x_{n + 1} \right) \right|= \frac{1}{2}\left| \left( x_{n + 1} - x_{n} \right)\left( y_{n + 2} - y_{n + 1} \right) - \left( y_{n + 1} - y_{n} \right)\left( x_{n + 2} - x_{n + 1} \right) \right|= \frac{1}{2}\left| \left( x_{n + 1}y_{n + 2} - y_{n + 1}x_{n + 2} \right) + \left( x_{n}y_{n + 1} - y_{n}x_{n + 1} \right) - \left( x_{n}y_{n + 2} - y_{n}x_{n + 2} \right) \right|= \frac{1}{2}\left| \frac{9}{2}\left( \frac{1 - k}{1 + k} - \frac{1 + k}{1 - k} \right) + \frac{9}{2}\left( \frac{1 - k}{1 + k} - \frac{1 + k}{1 - k} \right) - \frac{9}{2}\left( \left( \frac{1 - k}{1 + k} \right)^{2} - \left( \frac{1 + k}{1 - k} \right)^{2} \right) \right|\).\par
...
所以\(\overrightarrow{P_{n}P_{n + 3}}\)和\(\overrightarrow{P_{n + 1}P_{n + 2}}\)平行，这就得到\(S_{\bigtriangleup P_{n}P_{n + 1}P_{n + 2}} = S_{\bigtriangleup P_{n + 1}P_{n + 2}P_{n + 3}}\)，即\(S_{n} = S_{n + 1}\).\par
