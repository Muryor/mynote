\examxtitle{nanjing_2026_sep_preprocessed}
...
\begin{choices}
  若\(m = 1\)，则\(\left( \overset{\rightarrow}{a} - \overset{\rightarrow}{c} \right)\bot\overset{\rightarrow}{b}\)
...
\end{tikzpicture}
% IMAGE_TODO_END id=nanjing_2026_sep-Q14-img1\(\frac{3}{2}\).}
\end{question}

\section{解答题}

\begin{question}
若每次抽取后都放回，设取到黄球的个数为\(X\)，求\(P(X \geq 1)\)；
...
\begin{question}
已知\(a_{n} = n^{2} + n + 1\)，证明：\(\left\{ a_{n} \right\}\)的差分数列为等差数列；
...
% CONTEXT_BEFORE: {2}\) \(\Rightarrow\) \(|AB| = 6\). 如图：
...
%
\begin{question}
当\(a = 1\)时，若直线\(y = - x + b\)是曲线\(y = f(x)\)的一条切线，求\(b\)的值；
