...
height="1.3888888888888888e-2in"}**江苏省南京市2026届高三上学期9月学情调研数学试题**
...
【详解】样本数据1，2，2，2，3，5的平均数为$$\frac{5}{2}$$，极差为4，中位数为2，
...
3．已知$$a \in \text{R}$$，若集合$$M = \left\{ 0,a \right\},N = \left\{ - 2,0,1 \right\}$$，则"$$a = 1$$"是"$$M \subseteq N$$"的（　　）
...
【分析】利用集合的包含关系求出$$a$$，再利用充分条件、必要条件的定义判断即得.
...
【详解】$$(x - 1)^{6}$$展开式的通项为$$T_{r + 1} = \text{C}_{6}^{r}x^{6 - r}( - 1)^{r}$$，
...
5．要得到函数$$y = 3 \cdot 2^{x}$$的图象，只需将函数$$y = 2^{x}$$的图象(   )

> A．向左平移$$\text{log}_{2}3$$个单位长度
>
> B．向右平移$$\text{log}_{2}3$$个单位长度
>
> C．向上平移$$\text{log}_{2}3$$个单位长度
>
> D．向下平移$$\text{log}_{2}3$$个单位长度
...
【分析】由题可得：$$y = 3 \cdot 2^{x} = 2^{x + \log_{2}3}$$,利用图像平移的规则求解即可.

【详解】由题可得：$$y = 3 \cdot 2^{x} = 2^{x + \log_{2}3}$$,所以只需将函数$$y = 2^{x}$$的图象向左平移$$\text{log}_{2}3$$个单位长度得到函数$$y = 3 \cdot 2^{x}$$的图象；

故选：A

6．设等比数列$$\left\{ a_{n} \right\}$$的前$$n$$项和为$$S_{n}$$，若$$S_{2} = 2，S_{4} = 6$$，则$$a_{5} + a_{6} =$$（　　）
...
【分析】根据等比数列片段和的性质即可得到$$2,4,S_{6} - S_{4}$$成等比数列，再计算即可得到答案.

【详解】等比数列$$\left\{ a_{n} \right\}$$中，$$S_{2},S_{4} - S_{2},S_{6} - S_{4}$$成等比数列，
...
7．已知点$$A( - 1,1),B(3,3)$$，线段$$AB$$为$$\odot M$$的一条直径．设过点$$C(2, - 1)$$且与$$\odot M$$相切的两条直线的斜率分别为$$k_{1}，k_{2}$$，则$$k_{1} + k_{2} =$$（　　）

> A．$$- \frac{3}{2}$$ B．$$- \frac{2}{3}$$ C．$$\frac{2}{3}$$
> D．$$\frac{3}{2}$$
...
【分析】根据$$A( - 1,1),B(3,3)$$可得圆心和半径，进而根据点到直线的距离公式，结合相切即可求解.

【详解】由于点$$A( - 1,1),B(3,3)$$，线段$$AB$$为$$\odot M$$的一条直径，故圆心$$M\left( \frac{- 1 + 3}{2},\frac{1 + 3}{2} \right)$$,即$$M(1,2)$$，圆的半径为$$r = \frac{1}{2}|AB| = \frac{1}{2}\sqrt{( - 1 - 3)^{2} + (1 - 3)^{2}} = \sqrt{5}$$,
...
8．数学中有许多形状优美、寓意美好的曲线，如星形线等．某星形线如图所示，已知该曲线上一点$$P\left( x_{0}，y_{0} \right)$$的坐标可以表示为$$\left( a\text{cos}^{3}\theta ，a\text{sin}^{3}\theta \right)(a > 0)$$，若$$x_{0}y_{0} = \frac{8a^{2}}{125}$$，且$$x_{0} + y_{0} = \frac{9}{5}$$，则$$a =$$（　　）

![@@@74994822-7a76-4465-b523-6b19e9aef9ba](/Users/muryor/code/mynote/word_to_tex/output/figures/raw/media/image2.png){width="1.5416666666666667in"
height="1.46875in"}
...
【知识点】sinα±cosα和sinα·cosα的关系、由方程研究曲线的性质

【分析】根据同角三角函数的基本关系及立方和公式化简求值即可.

【详解】

$$\because x_{0}y_{0} = \frac{8a^{2}}{125}$$，$$x_{0} + y_{0} = \frac{9}{5}$$，
...
> C．$$\text{ln}(b - a) > 0$$ D．$$a^{3} < b^{3}$$

【答案】BD

【难度】0.85

【知识点】由不等式的性质比较数（式）大小、作差法比较代数式的大小、比较对数式的大小

【分析】对于AB可用作差比较法比较大小即可判断，对于C，根据对数函数性质，易知当$$0 < b - a < 1$$时，$$\text{ln}(b - a) < 0$$从而排除C项；对于D，可用不等式的性质直接推得．

【详解】对于A，由$$a < b < 0$$，则$$a - b < 0,a + b < 0$$，由$$a^{2} - b^{2} = (a - b)(a + b) > 0$$，
...
10．已知向量$$\overrightarrow{a} = (2,4),\overset{\rightarrow}{b} = \left( m,\frac{1}{m} \right),\overset{\rightarrow}{c} = (3,3)$$，则下列说法正确的是（　　）

> A．若$$m = 1$$，则$$\left( \overset{\rightarrow}{a} - \overset{\rightarrow}{c} \right)\bot\overset{\rightarrow}{b}$$
> B．若$$\overset{\rightarrow}{a}\text{//}\overset{\rightarrow}{b}$$，则$$m = \frac{\sqrt{2}}{2}$$
>
> C．$$\overset{\rightarrow}{a}$$在$$\overset{\rightarrow}{c}$$上的投影向量为$$\overset{\rightarrow}{c}$$
> D．$$\left| \overset{\rightarrow}{b} - \overset{\rightarrow}{c} \right|$$的最小值为$$\sqrt{7}$$

【答案】ACD

【难度】0.65

【知识点】由向量共线（平行）求参数、数量积的坐标表示、坐标计算向量的模、求投影向量

【分析】根据数量积计算可判断A，由共线向量的坐标表示可判断B，根据投影向量的计算公式可判断C，利用向量模的坐标表示及均值不等式、二次函数的最值可判断D.

【详解】若$$m = 1$$，$$\overset{\rightarrow}{a} - \overset{\rightarrow}{c} = ( - 1,1)$$，$$\overset{\rightarrow}{b} = (1,1)$$，所以$$\left( \overset{\rightarrow}{a} - \overset{\rightarrow}{c} \right) \cdot \overset{\rightarrow}{b} = - 1 \times 1 + 1 \times 1 = 0$$，
...
11．已知函数$$f(x) = \sqrt{|x + 1|} - \sqrt{|x - 1|}$$，则（　　）．

> A．$$f( - x) + f(x) = 0$$
>
> B．$$f(x)$$在$$( - 1,1)$$上单调递减
>
> C．$$f(x + 1) - f(x) \leq \sqrt{2}$$
>
> D．$$g(x) = f(x) - \text{sin}x$$在$$( - 1,1)$$上有且仅有1个零点

【答案】ACD

【难度】0.4

【知识点】函数奇偶性的定义与判断、利用导数研究不等式恒成立问题、利用导数研究函数的零点

【分析】对于A，代入计算即可得到；对于B，由$$x \in ( - 1,1)$$时，$$f(x) = \sqrt{x + 1} - \sqrt{1 - x}$$，结合初等函数单调性可直接判断；对于C，令$$h(x) = f(x + 1) - f(x)$$，分析可得$$h(x)$$关于$$x = - \frac{1}{2}$$对称，当$$x \geq 1$$时，易证$$h(x) < 0$$，当$$0 \leq x < 1$$和$$- \frac{1}{2} \leq x < 0$$时，利用导数判断单调性，结合单调性可证明不等式；对于D，对$$g(x)$$进行求导，结合基本不等式，可得$$g(x)$$在$$( - 1,1)$$上单调递增，且$$g(0) = 0$$，故$$g(x)$$在$$( - 1,1)$$上有且仅有1个零点．

【详解】对于A，$$\because f( - x) = \sqrt{| - x + 1|} - \sqrt{| - x - 1|} = \sqrt{|x - 1|} - \sqrt{|x + 1|}$$，
...
12．已知椭圆$$\frac{x^{2}}{m + 2} + \frac{y^{2}}{m - 1} = 1$$的离心率为$$\frac{1}{2}$$，则实数$$m =$$
．
...
【分析】判断椭圆为焦点在*x*轴上的椭圆，从而确定$$a^{2},b^{2}$$，再结合离心率公式及*a*、*b*、*c*的关系即可列式解答．

【详解】$$e^{2} = \frac{c^{2}}{a^{2}} = \frac{a^{2} - b^{2}}{a^{2}} = 1 - \frac{b^{2}}{a^{2}} = \frac{1}{4} \Rightarrow 3a^{2} = 4b^{2}$$．
...
13．记$$\bigtriangleup ABC$$的内角$$A,B,C$$的对边分别为$$a,b,c$$，若$$B = \frac{\text{π}}{4},c = 4,b\text{sin}A = 1$$，则$$b =$$
．
...
【详解】由正弦定理$$\frac{a}{\sin A} = \frac{b}{\sin B}$$可得$$a = \frac{b\sin A}{\sin B} = \frac{1}{\frac{\sqrt{2}}{2}} = \sqrt{2}$$，

则$$b^{2} = a^{2} + c^{2} - 2ac\cos B = 2 + 16 - 8\sqrt{2} \times \frac{\sqrt{2}}{2} = 10$$，即$$b = \sqrt{10}$$.

故答案为：$$\sqrt{10}$$.

14．已知球$$O$$的半径为$$3，P，Q$$是球面上两点，过$$P，Q$$的平面与球面的交线为圆$$O_{1}$$，且$$P，Q，O，O_{1}$$四点不共面．若平面$$PQO_{1}$$与平面$$PQO$$的夹角为$$60^{\circ}$$，则四面体$$PQOO_{1}$$体积的最大值为
．
...
【分析】作出二面角的平面角，设$$O_{1}M = t$$，再写出体积表达式$$V = \frac{\sqrt{3}}{3}\sqrt{9t^{4} - 4t^{6}}$$，求导即可得到其最值.

【详解】取$$PQ$$中点$$M$$，因为$$O_{1}P = O_{1}Q$$，则$$Q_{1}M\bot PQ$$，同理可得$$OM\bot PQ$$，
...
![@@@9f7a4efb-81f8-442b-b4a7-00f1ef94652d](/Users/muryor/code/mynote/word_to_tex/output/figures/raw/media/image3.png){width="1.7916666666666667in"
...
(1)若每次抽取后都放回，设取到黄球的个数为$$X$$，求$$P(X \geq 1)$$；

(2)若每次抽取后都不放回，设取到黄球的个数为$$Y$$，求$$Y$$的分布列和数学期望．
...
【知识点】独立重复试验的概率问题、超几何分布的分布列

【分析】（1）根据二项分布的有关公式求值计算.

（2）根据超几何分布的公式计算求值.

【详解】（1）每次抽取后都放回，则取到黄球的个数$$X \sim B\left( 2,\frac{3}{8} \right)$$，
...
16．对于数列$$\left\{ a_{n} \right\}$$，记$$\bigtriangleup a_{n} = a_{n + 1} - a_{n},n \in \text{N}^{\text{*}}$$，称数列$$\left\{ \bigtriangleup a_{n} \right\}$$为数列$$\left\{ a_{n} \right\}$$的差分数列．

(1)已知$$a_{n} = n^{2} + n + 1$$，证明：$$\left\{ a_{n} \right\}$$的差分数列为等差数列；

(2)已知$$\left\{ a_{n} \right\}$$的差分数列为$$\left\{ \frac{2n^{2} + 2n + 1}{n^{2} + n} \right\} ，a_{1} = 1$$，求$$\left\{ a_{n} \right\}$$的通项公式．
...
【知识点】累加法求数列通项、判断等差数列、裂项相消法求和、数列新定义

【分析】（1）根据定义可求$$\bigtriangleup a_{n}$$，再利用定义法可证$$\left\{ a_{n} \right\}$$的差分数列为等差数列；

（2）利用累加法可求$$\left\{ a_{n} \right\}$$的通项公式．

【详解】（1）$$\bigtriangleup a_{n} = (n + 1)^{2} + (n + 1) + 1 - \left( n^{2} + n + 1 \right) = 2n + 2$$，其中$$n \geq 1$$，
...
17．如图，直三棱柱$$ABC - A_{1}B_{1}C_{1}$$中，$$M，N$$分别为$$AB$$和$$B_{1}C_{1}$$的中点．

![@@@d25a030a-2303-43c7-a0c7-66b6a73cbd2d](/Users/muryor/code/mynote/word_to_tex/output/figures/raw/media/image4.png){width="1.7916666666666667in"
height="1.6041666666666667in"}

(1)证明：$$MN\text{//}$$平面$$ACC_{1}A_{1}$$；

(2)若$$AC = 2\sqrt{2},AB = 2\sqrt{3},\angle ACB = 90{^\circ},MN\bot A_{1}C$$，求$$A_{1}C$$与平面$$CMN$$所成角的正弦值．
...
【分析】（1）取$$A_{1}C_{1}$$的中点为$$S$$，连接$$SN,AS$$，可证$$AS//NM$$，再利用线面平行的判定定理可证$$MN\text{//}$$平面$$ACC_{1}A_{1}$$；

（2）建立如图所示的空间直角坐标系，根据$$MN\bot A_{1}C$$可求$$AA_{1}$$的长，再求出平面$$CMN$$的法向量后可求线面角的正弦值.

【详解】（1）取$$A_{1}C_{1}$$的中点为$$S$$，连接$$SN,AS$$，
...
![@@@210c8f48-b899-4bc5-a203-46aaf17cb775](/Users/muryor/code/mynote/word_to_tex/output/figures/raw/media/image5.png){width="1.7916666666666667in"
height="1.59375in"}
...
![@@@2913820e-4dc6-483a-b9f2-9baac169de9a](/Users/muryor/code/mynote/word_to_tex/output/figures/raw/media/image6.png){width="1.9375in"
...
(3)是否存在$$x$$轴上的定点$$M$$，使得$$\overrightarrow{MA} \cdot \overrightarrow{MB}$$为定值？若存在，求出点$$M$$的坐标；若不存在，请说明理由．
...
【知识点】根据a、b、c求双曲线的标准方程、求双曲线中的弦长、双曲线中存在定点满足某条件问题

【分析】（1）根据双曲线中$$a,b,c$$的意义和关系，可求$$a,b,c$$的值，得到双曲线的方程.

（2）先根据双曲线的定义，求出弦$$AB$$的长度；设直线$$l$$：$$x = ty + 2$$，与双曲线方程联立，利用弦长公式，可求$$t$$的值，即得直线方程.

（3）假设存在$$x$$轴上的定点$$M$$
$$(m,0)$$，使得$$\overrightarrow{MA} \cdot \overrightarrow{MB}$$为定值.结合（2）中的结论，根据$$\overrightarrow{MA} \cdot \overrightarrow{MB}$$为定值，可求$$m$$的值.

【详解】（1）因为$$\left| F_{1}F_{2} \right| = 4$$ $$\Rightarrow$$
$$2c = 4$$，所以$$c = 2$$，
...
![@@@0df61d26-0be6-4fb5-bb2b-444d7221a1c2](/Users/muryor/code/mynote/word_to_tex/output/figures/raw/media/image7.png){width="1.84375in"
height="1.53125in"}
...
19．已知函数$$f(x) = \text{ln}(ax) + \frac{1}{x} + a(x - 1)$$，其中$$a \neq 0$$．

(1)当$$a = 1$$时，若直线$$y = - x + b$$是曲线$$y = f(x)$$的一条切线，求$$b$$的值；

(2)讨论$$f(x)$$的单调性；

(3)若集合$$\{ x \mid f(x) < 1,x \in \text{Z}\}$$中有且仅有一个元素，求$$a$$的取值范围．
...
【知识点】已知切线（斜率）求参数、函数单调性、极值与最值的综合应用、利用导数求函数（含参）的单调区间

【分析】（1）根据导函数的几何意义，求出切线斜率为$$- 1$$时的切点，进而求出参数值；

（2）根据函数导数与函数单调性的关系，求出函数导数，根据参数的范围，求出函数定义域，进而根据参数范围求出函数单调性；

（3）根据参数的范围，结合函数单调性，判断不等式只有一个整数解的情况，列出不等式组，求出参数范围.

【详解】（1）当$$a = 1$$时，$$f(x) = \ln x + \frac{1}{x} + x - 1(x > 0)$$，则$$f'(x) = \frac{1}{x} - \frac{1}{x^{2}} + 1$$，
