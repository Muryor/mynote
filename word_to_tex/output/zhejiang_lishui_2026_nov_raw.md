![](/Users/muryor/code/mynote/word_to_tex/output/figures/raw/media/image1.png){width="1.3888888888888888e-2in"
height="1.3888888888888888e-2in"}**浙江省丽水、湖州、衢州三地市2026届高三上学期11月教学质量检测数学试题**

**一、单选题**

1．已知复数$$z$$，若$$\frac{z}{2 + \text{i}} = \text{i}$$（$$\text{i}$$为虚数单位），则$$|z| =$$（　　）
...
【分析】利用复数的运算法则求出$$z$$，再根据复数模长计算公式可得答案.
...
2．已知集合$$A = \{ x \mid - 2 \leq x < 1\},B = \left\{ m,3 \right\}$$，且$$A \cap B$$的元素个数是一个，则实数$$m$$的取值范围是（　　）

> A．$$( - 2,1)$$ B．$$\lbrack - 2,1\rbrack$$ C．$$\lbrack - 2,1)$$
> D．$$( - 2,1\rbrack$$
...
【详解】由$$A \cap B$$的元素个数是一个，且$$3 \notin A$$，得$$m \in A$$，则$$- 2 \leq m < 1$$，

所以实数$$m$$的取值范围是$$\lbrack - 2,1)$$.

故选：C

3．已知$$F_{1},F_{2}$$为双曲线$$C：\frac{x^{2}}{a^{2}} - \frac{y^{2}}{b^{2}} = 1(a > 0,b > 0)$$的左右焦点，点$$A$$的坐标为$$(0,2b)$$．若$$\bigtriangleup AF_{1}F_{2}$$为等边三角形，则双曲线$$C$$的离心率是（　　）
...
【分析】由题可得$$\tan\angle AF_{1}O = \frac{2b}{c} = \sqrt{3}$$，即$$\frac{b}{c} = \frac{\sqrt{3}}{2}$$，再根据$$e^{2} = \frac{c^{2}}{a^{2}} = \frac{c^{2}}{c^{2} - b^{2}} = \frac{1}{1 - \left( \frac{b}{c} \right)^{2}}$$求解即可．

【详解】$$\because \bigtriangleup AF_{1}F_{2}$$为等边三角形，$$O$$为$$F_{1}F_{2}$$的中点，

![@@@6aeafe47-af7d-4860-a8d5-3862f785d979](/Users/muryor/code/mynote/word_to_tex/output/figures/raw/media/image2.png){width="1.34375in"
height="1.8125in"}$$\therefore\tan\angle AF_{1}O = \frac{2b}{c} = \sqrt{3}$$，则$$\frac{b}{c} = \frac{\sqrt{3}}{2}$$，
...
4．已知$$x，y \in \text{R}$$，则下列条件中使$$x > y$$成立的充要条件是（　　）
...
【知识点】探求命题为真的充要条件、对数函数单调性的应用、由指数函数的单调性解不等式

【分析】逐一分析各选项条件和$$x > y$$能否互推即可求解.

【详解】对于A，当$$x = - 2,y = 1$$时，满足$$|x| > y$$，但不满足$$x > y$$，
...
5．定义在$$R$$上的两个函数$$f(x),g(x)$$，恒有$$f^{3}(x) = g\left( x^{2} \right)$$，则（　　）

> A．$$f(x)$$为奇函数 B．$$f(x)$$为偶函数
>
> C．$$g(x)$$为奇函数 D．$$g(x)$$为偶函数
...
【详解】由$$f^{3}(x) = g\left( x^{2} \right)$$，则$$f^{3}( - x) = g\left\lbrack ( - x)^{2} \right\rbrack = g\left( x^{2} \right) = f^{3}(x)$$，
...
6．若函数$$y = \text{sin}\left| \omega x + \frac{\text{π}}{6} \right|$$的图象向右平移$$\frac{\text{π}}{4}$$个单位后得到的图象关于$$y$$轴对称，则实数$$\omega$$可以是（　　）
...
【分析】分析可知函数$$y = \text{sin}\left| \omega x + \frac{\text{π}}{6} \right|$$关于直线$$x = - \frac{\text{π}}{4}$$对称，分$$\omega = 0$$和$$\omega \neq 0$$两种情况，结合函数$$y = \sin|t|$$的对称性分析求解即可.

【详解】因为函数$$y = \text{sin}\left| \omega x + \frac{\text{π}}{6} \right|$$的图象向右平移$$\frac{\text{π}}{4}$$个单位后得到的图象关于$$y$$轴对称，
...
7．已知三棱锥$$S - ABC$$，满足$$SA = SB = SC$$，且$$SA$$，$$SB$$，$$SC$$两两垂直．在底面$$\bigtriangleup ABC$$内有一动点$$P$$到三个侧面的距离依次成等差数列，则点$$P$$的轨迹是（　　）
...
【知识点】等差中项的应用、立体几何中的轨迹问题

【分析】推导出$$V_{S - PAB} + V_{S - PAC} = 2V_{S - PBC}$$，进而可得出$$S_{\bigtriangleup PAB} + S_{\bigtriangleup PAC} = 2S_{\bigtriangleup PBC}$$，设点$$P$$到边$$AB$$、$$BC$$、$$AC$$的距离分别为$$d_{1}$$、$$d_{2}$$、$$d_{3}$$，设等边$$\bigtriangleup ABC$$的边长为$$a$$，可得出$$d_{1} + d_{2} + d_{3} = \frac{\sqrt{3}}{2}a$$以及$$d_{1} + d_{3} = 2d_{2}$$，求出$$d_{2}$$的值，即可得出结论.

【详解】在三棱锥$$S - ABC$$中，$$SA = SB = SC$$，且$$SA$$，$$SB$$，$$SC$$两两垂直，
...
![@@@a1a78d87-b204-4fe8-b78e-081d3ecda4a8](/Users/muryor/code/mynote/word_to_tex/output/figures/raw/media/image3.png){width="1.6666666666666667in"
...
8．若关于$$x$$的方程$$\left( \text{e}^{x} - tx \right)\left( x - t\text{ln}x \right) = 0(t \in R)$$恰有四个不同的实根$$a,b,c,d(a < b < c < d)$$，则（　　）

> A．$$a + d < b + c$$ B．$$a + d = b + c$$ C．$$ad < bc$$
> D．$$ad = bc$$
...
【知识点】根据函数零点的个数求参数范围、用导数判断或证明已知函数的单调性、函数单调性、极值与最值的综合应用、利用导数研究方程的根

【分析】令$$\left( \text{e}^{x} - tx \right)\left( x - t\text{ln}x \right) = 0$$，可得$$t = \frac{\text{e}^{x}}{x}$$或$$t = \frac{x}{\ln x}$$，构造函数$$f(x) = \frac{\text{e}^{x}}{x}$$，则$$f\left( \ln x \right) = \frac{x}{\ln x}$$，结合导数可得函数$$f(x)$$性质，从而得到$$t = \frac{\text{e}^{x}}{x}$$与$$t = \frac{x}{\ln x}$$的根的关系，即可得$$a,b,c,d$$间关系，从而可得$$ad$$与$$bc$$间关系；再借助作差法，结合导数计算可得$$a + d$$与$$b + c$$关系，即可得解.

【详解】由$$\left( \text{e}^{x} - tx \right)\left( x - t\text{ln}x \right) = 0$$，则$$\text{e}^{x} - tx = 0$$或$$x - t\text{ln}x = 0$$，
...
9．已知随机变量$$\xi \sim N\left( 0,\sigma^{2} \right),\varphi(x) = P(\xi \leq x)$$，则下列等式正确的是（　　）

> A．$$\varphi( - x) = 1 - \varphi(x)$$
> B．$$P\left( |\xi| \leq 1 \right) = 1 - 2\varphi(1)$$
>
> C．$$P\left( |\xi| \leq 1 \right) = 2\varphi(1) - 1$$
> D．$$P\left( |\xi| > 1 \right) = 2 - 2\varphi(1)$$
...
【详解】因为随机变量$$\xi \sim N\left( 0,\sigma^{2} \right)$$，
...
10．设抛物线$$C：y^{2} = 2px(p > 0)$$的焦点为$$F$$，准线为$$l$$，过点$$F$$的直线交$$C$$于$$A$$，$$B$$两点，以$$F$$为圆心，$$|FA|$$为半径的圆交$$l$$于$$M，N$$两点．若$$AM\bot l，|FA| = 6$$，则（　　）

> A．$$p = 2$$ B．直线$$AF$$的斜率是$$\pm \sqrt{3}$$
>
> C．$$|AB| = 8$$ D．$$\bigtriangleup AMN$$的面积是$$18\sqrt{3}$$
...
【分析】对于A,利用几何关系求出焦点和准线之间的距离即可；对于B，利用几何关系求出直线$$AF$$的倾斜角即可；对于C，利用弦长公式求解即可；对于D，利用三角形面积公式求解即可.

【详解】对于A，以$$F$$为圆心，$$|FA|$$为半径的圆交准线$$l$$于$$M，N$$两点,且$$AM\bot l,$$
...
![@@@a9ebced2-0c14-414c-be5a-bb8190e93515](/Users/muryor/code/mynote/word_to_tex/output/figures/raw/media/image4.png){width="1.5in"
height="1.5416666666666667in"}

11．在$$\bigtriangleup ABC$$中，若$$C > B$$，且$$\text{sin}^{2}B + \text{cos}^{2}C - \text{sin}B\text{cos}C = \frac{3}{4}$$，则（　　）

> A．$$C = \frac{\text{π}}{2}$$ B．$$\text{sin}A = \frac{1}{2}$$
>
> C．$$\text{sin}A = \text{cos}C$$
> D．$$\text{2sin}B - \text{cos}C$$的最大值是$$\sqrt{3}$$

【答案】BD

【难度】0.4

【知识点】二倍角的余弦公式、三角恒等变换的化简问题

【分析】利用降幂公式以及三角恒等变换可得$$\left( 2\sin A - 1 \right)\left\lbrack 2\sin(B - C) - 1 \right\rbrack = 0$$，结合已知可得$$\sin(B - C) < 0$$，可求得$$\text{sin}A = \frac{1}{2}$$判断B；求得$$A$$，进而计算可判断ACD.

【详解】由$$\text{sin}^{2}B + \text{cos}^{2}C - \text{sin}B\text{cos}C = \frac{3}{4}$$，可得$$\frac{\text{1} - \cos 2B}{2} + \frac{\text{1} + \text{cos}2C}{2} - \text{sin}B\text{cos}C = \frac{3}{4}$$，
...
12．$$\left( x^{3} - 1 \right)\left( x^{2} + \frac{1}{x} \right)^{6}$$展开式中的常数项是
．
...
【分析】根据题意，先求得二项式$$\left( x^{2} + \frac{1}{x} \right)^{6}$$的展开式的通项公式为$$T_{r + 1} = C_{6}^{r}x^{12 - 3r}$$，进而求得展开式的常数项，得到答案.

【详解】由二项式$$\left( x^{2} + \frac{1}{x} \right)^{6}$$的展开式的通项公式为$$T_{r + 1} = C_{6}^{r}{(x^{2})}^{6 - r} \cdot {(\frac{1}{x})}^{r} = C_{6}^{r}x^{12 - 3r},r = 0,1,\cdots,6$$，
...
所以所求展开式的常数项为$$C_{6}^{5} + ( - 1) \cdot C_{6}^{4} = 6 - 15 = - 9$$.

故答案为：$$- 9$$.

13．已知平面向量$$\overrightarrow{a}，\overrightarrow{b}$$满足$$\left| \overrightarrow{a} \right| = \sqrt{3}，\left| \overrightarrow{a} - \overrightarrow{b} \right| = 1$$，则$$\left| \overrightarrow{b} \right|$$的最大值是
．
...
$$\left| \overrightarrow{b} \right| \leq \sqrt{3} + 1$$

故答案为：$$\sqrt{3} + 1$$

14．在Rt$$\bigtriangleup ABC$$中，$$C = \frac{\text{π}}{2},AC = 1,BC = \sqrt{3},D$$是$$AB$$的中点，把$$\bigtriangleup BCD$$沿$$CD$$翻折到$$\bigtriangleup B_{1}CD$$，设二面角$$B_{1} - CD - A$$的平面角为$$\theta$$，若$$\theta \in \left\lbrack \frac{\text{π}}{3},\frac{\text{π}}{2} \right\rbrack$$，则三棱锥$$B_{1} - ACD$$外接球表面积的范围是
．

【答案】$$\left\lbrack 4\text{π},\frac{13\text{π}}{3} \right\rbrack$$

【难度】0.4

【知识点】由导数求函数的最值（不含参）、球的表面积的有关计算、多面体与球体内切外接问题、求二面角

【分析】利用球心在过$$\bigtriangleup ACD$$和$$\bigtriangleup CDB_{1}$$外接圆圆心且垂直于平面$$ACD$$和平面$$CDB_{1}$$的垂线的交线上找到球心位置，利用题中所给条件建立外接球半径与二面角$$B_{1} - CD - A$$的平面角为$$\theta$$的关系式即可分析计算求解.

【详解】由题可得$$AB = \sqrt{\left( \sqrt{3} \right)^{2} + 1^{2}} = 2$$，所以$$AC = CD = AD = BD = B_{1}D = 1,CB_{1} = \sqrt{3}$$，
...
分别过$$O_{1},O_{2}$$作平面$$ACD$$和平面$$CDB_{1}$$的垂线，则球心均在两垂线上，两垂线的交点即为球心*O*，
...
若$$\frac{\text{π}}{3} \leq \theta < \frac{\text{π}}{2}$$，如图，连接$$O_{1}O_{2}$$，则$$O_{1}O_{2}$$与$$O_{2}G$$相交于平面$$CDB_{1}$$一点*H*，

![@@@6688a7fe-0abc-49d5-9d56-7ba112a13dc8](/Users/muryor/code/mynote/word_to_tex/output/figures/raw/media/image5.png){width="1.7291666666666667in"
height="1.5729166666666667in"}

则$$\angle GHO_{1} = \angle O_{2}HO,\angle GO_{1}H = \angle OO_{2}H = 90{^\circ},$$所以$$\angle O_{2}OH = \angle AGO_{2} = \theta$$，

设三棱锥$$B_{1} - ACD$$外接球半径为*R*，
...
![@@@660b98d5-aeb1-4eae-b5c4-9e93edbc598b](/Users/muryor/code/mynote/word_to_tex/output/figures/raw/media/image6.png){width="1.5416666666666667in"
height="1.5208333333333333in"}

所以三棱锥$$B_{1} - ACD$$外接球表面积最小值为$$4\text{π}R^{2} = 4\text{π}$$，最大值为$$4\text{π}R^{2} = 4\text{π} \times \frac{13}{12} = \frac{13\text{π}}{3}$$.

故答案为：$$\left\lbrack 4\text{π},\frac{\text{13π}}{3} \right\rbrack$$

**四、解答题**

15．已知数列$$\left\{ a_{n} \right\}$$满足$$a_{1} = 1,a_{2} = \frac{2}{3},\frac{a_{n + 1} - a_{n}}{a_{n}} = \frac{a_{n + 2} - a_{n + 1}}{a_{n + 2}}\left( n \in N^{\text{*}} \right)$$．

(1)证明：数列$$\left\{ \frac{1}{a_{n}} \right\}$$为等差数列；

(2)求数列$$\left\{ a_{n}a_{n + 1} \right\}$$的前$$n$$项和$$S_{n}$$．
...
【分析】（1）将$$\frac{a_{n + 1} - a_{n}}{a_{n}} = \frac{a_{n + 2} - a_{n + 1}}{a_{n + 2}}$$变形化简后结合等差数列定义即可得；

（2）利用裂项相消法计算即可得.

【详解】（1）由$$\frac{a_{n + 1} - a_{n}}{a_{n}} = \frac{a_{n + 2} - a_{n + 1}}{a_{n + 2}}$$，得$$\frac{a_{n + 1}}{a_{n}} - 1 = 1 - \frac{a_{n + 1}}{a_{n + 2}}$$，
...
16．如图，在三棱台$$ABC - A_{1}B_{1}C_{1}$$中，平面$$AA_{1}C_{1}C\bot$$平面$$ABC,AA_{1} = A_{1}C_{1} = C_{1}C = 2$$，$$AC = 4,A_{1}B = 2\sqrt{2},AB = BC$$．

![@@@267e4ff991af46dda0cfb11c4e6d1076](/Users/muryor/code/mynote/word_to_tex/output/figures/raw/media/image7.png){width="1.8020833333333333in"
height="1.2916666666666667in"}

(1)求证：$$A_{1}B\bot AC_{1}$$；

(2)求平面$$ABC_{1}$$与平面$$BCC_{1}$$夹角的余弦值．
...
【分析】（1）取$$AC$$的中点$$O$$，连接$$BO,A_{1}O,C_{1}O$$，利用面面垂直的性质可得$$BO\bot$$平面$$AA_{1}C_{1}C$$，进而可证$$AC_{1}\bot BO$$，结合$$AC_{1}\bot A_{1}O$$，可证$$AC_{1}\bot$$平面$$A_{1}OB$$，可证结论；

（2）建立空间直角坐标系，求得平面$$ABC_{1}$$和平面$$CBC_{1}$$的一个法向量，利用向量法可求得平面$$ABC_{1}$$和$$BCC_{1}$$夹角的余弦值.

【详解】（1）取$$AC$$的中点$$O$$，连接$$BO,A_{1}O,C_{1}O$$，
...
![@@@5d23cf5de25a42cb9a4f05bae27ded91](/Users/muryor/code/mynote/word_to_tex/output/figures/raw/media/image8.png){width="2.3958333333333335in"
...
(1)按照上述规则摸球3次．当第1次选中的是甲袋，求摸到红球的个数$$X$$的分布列及期望$$E(X)$$；

(2)按照上述规则进行连续摸球，若摸到2次红球则停止摸球．求3次之内（含3次）停止摸球的概率．

【答案】(1)分布列见解析，$$\frac{3}{4}$$
...
18．已知$$A( - 5,0),B\left( 4,\frac{12}{5} \right)$$是椭圆$$C:\frac{x^{2}}{a^{2}} + \frac{y^{2}}{b^{2}} = 1(a > b > 0)$$上的两点．

(1)求椭圆$$C$$的标准方程；

(2)已知$$P,Q$$是椭圆$$C$$上的两动点，且$$P,Q$$的横坐标之和为$$\frac{50}{9}$$，设直线$$l$$为线段$$PQ$$的中垂线，过点$$A$$作直线$$AM\bot l$$，垂足为$$M$$．求垂足$$M$$横坐标$$x_{M}$$的取值范围，并求$$M$$的轨迹方程．

【答案】(1)$$\frac{x^{2}}{\text{25}}\text{+}\frac{y^{2}}{16} = 1$$；

(2)$$x_{M} \in \lbrack - 5, - \frac{1}{3})$$，$${(x + 2)}^{2} + y^{2} = 9$$且$$x \in \lbrack - 5, - \frac{1}{3})$$.

【难度】0.4

【知识点】轨迹问题------圆、求平面轨迹方程、根据椭圆过的点求标准方程

【分析】（1）根据椭圆上两点代入方程求解$$a,b$$的值即可得椭圆方程；

（2）设$$P(x_{1},y_{1}),Q(x_{2},y_{2})$$，线段$$PQ$$的中点$$G(x_{0},y_{0})$$，分别讨论直线$$PQ$$的斜率是否存在，当斜率$$k$$存在时确定直线$$l$$的方程与直线$$PQ$$联立得$$M$$横坐标$$x_{M}$$与$$k$$的关系，结合函数得$$x_{M}$$的取值范围，结合圆的定义从而得$$M$$的轨迹方程.

【详解】（1）由题意$$\left\{ \begin{array}{r}
a = \text{5} \\
...
![@@@eed36597-0f39-4b2c-bfd7-860058066505](/Users/muryor/code/mynote/word_to_tex/output/figures/raw/media/image9.png){width="1.65625in"
height="1.40625in"}
...
所以$$x_{M} = - 5 + \frac{6}{k^{2} + 1} \in ( - 5, - \frac{1}{3})$$，综上①，②所得$$x_{M} \in \lbrack - 5, - \frac{1}{3})$$

所以垂足$$M$$轨迹方程是$${(x + 2)}^{2} + y^{2} = 9$$，且$$x \in \lbrack - 5, - \frac{1}{3})$$．

19．已知函数$$f(x) = \text{e}^{x}\text{cos}(ax)$$，$$a \in R$$，$$g(x) = x\text{sin}x$$（$$\text{e} \approx 2.718$$为自然对数的底数）．
...
(2)当$$a \in (0,2)$$时，求证：对任意的$$x \in \left\lbrack 0,\frac{\text{π}}{4} \right\rbrack$$，$$a\text{e}^{x} \cdot g'(x) \geq f(x) - f'(x)$$恒成立．

【答案】(1)（i）$$\left\lbrack 2k\text{π} - \frac{3\text{π}}{4},2k\text{π} + \frac{\text{π}}{4} \right\rbrack,k \in Z$$；（ii）$$\frac{77\text{e}^{22\text{π}} - 81\text{e}^{21\text{π}} + 3\text{e}^{2\text{π}} + \text{e}^{\text{π}}}{4\left( 1 - \text{e}^{\text{π}} \right)^{2}}$$

(2)证明见解析

【难度】0.15

【知识点】利用导数求函数的单调区间（不含参）、利用导数证明不等式、利用定义求等差数列通项公式、错位相减法求和

【分析】（1）（i）求导，利用导数结合余弦函数性质求单调区间即可；（ii）根据函数单调性和极值点分析可知数列$$\left\{ x_{n} \right\}$$是以首项为$$\frac{\text{π}}{4}$$，公差为$$\text{π}$$的等差数列，整理可得$$\frac{\sqrt{2}\text{e}^{\frac{3}{4}\text{π}}}{\text{π}}x_{n} \cdot \left| f\left( x_{n} \right) \right| = \left( n - \frac{3}{4} \right)\text{e}^{n\text{π}}$$，利用错位相减法运算求解；

（2）整理可得$$a\text{\,\,}\text{e}^{x} \cdot g'(x) - f(x) + f'(x) = a\text{e}^{x}\left\lbrack \sin x + x\cos x - \sin(ax) \right\rbrack$$，结合题中范围可得$$\sin(ax) \leq \sin(2x)$$，再结合$$x - \sin x \geq 0,x \in \left\lbrack 0,\frac{\text{π}}{4} \right\rbrack$$分析证明即可.

【详解】（1）由题可得：$$f'(x) = \text{e}^{x}\left( \cos x - \sin x \right) = \sqrt{2}\text{e}^{x}\cos\left( x + \frac{\text{π}}{4} \right)$$，
