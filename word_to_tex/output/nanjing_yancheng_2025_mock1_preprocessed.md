![](/Users/muryor/code/mynote/word_to_tex/output/figures/nanjing_yancheng_2025_mock1/media/image1.png){width="1.3888888888888888e-2in"
height="1.3888888888888888e-2in"}**江苏省南京市、盐城市2024-2025学年高三下学期第一次模拟考试数学试题**

学校:\_\_\_\_\_\_\_\_\_\_\_姓名：\_\_\_\_\_\_\_\_\_\_\_班级：\_\_\_\_\_\_\_\_\_\_\_考号：\_\_\_\_\_\_\_\_\_\_\_

# 一、单选题

1．设集合$$A = \left\{ x \mid x^{2} - 4 \leq 0 \right\},B = \left\{ x \mid x + a \leq 0 \right\}$$.若$$A \subseteq B$$，则实数$$a$$的取值范围是（    ）

> A．$$( - \infty,2)$$ B．$$( - \infty,2\rbrack$$
> C．$$( - \infty, - 2)$$ D．$$( - \infty, - 2\rbrack$$
...
【分析】直接解不等式确定集合$$A,B$$，再根据集合的基本关系确定参数范围即可.

【详解】由$$x^{2} - 4 \leq 0$$可得$$A = \lbrack - 2,2\rbrack$$，由$$x + a \leq 0$$可得$$B = ( - \infty, - a\rbrack$$，

又$$A \subseteq B$$，所以$$2 \leq - a$$，即$$a \leq - 2$$，故D正确.

故选：D

2．已知复数$$z$$满足$$\frac{1}{z + \text{i}} = \text{i}$$（$$\text{i}$$为虚数单位），则$$|z| =$$（    ）
...
【分析】首先根据复数代数形式的除法运算化简$$z$$，再计算其模即可.

【详解】因为$$\frac{1}{z + \text{i}} = \text{i}$$，所以$$z + \text{i} = \frac{1}{\text{i}} = \frac{\text{i}}{\text{i}^{2}} = - \text{i}$$，所以$$z = - 2\text{i}$$，

所以$$|z| = 2$$.

故选：B

3．已知$$\overrightarrow{a},\overrightarrow{b},\overrightarrow{c}$$均为单位向量.若$$\overrightarrow{a} = \overrightarrow{b} + \overrightarrow{c}$$，则$$\overrightarrow{b}$$与$$\overrightarrow{c}$$夹角的大小是（    ）

> A．$$\frac{\text{π}}{6}$$ B．$$\frac{\text{π}}{3}$$
> C．$$\frac{2\text{π}}{3}$$ D．$$\frac{5\text{π}}{6}$$
...
【分析】对$$\overrightarrow{a} = \overrightarrow{b} + \overrightarrow{c}$$两边同时平方，再结合单位向量的性质求出$$\overrightarrow{b} \cdot \overrightarrow{c}$$，最后根据向量数量积公式求出夹角.

【详解】已知$$\overrightarrow{a} = \overrightarrow{b} + \overrightarrow{c}$$，两边平方可得$${\overrightarrow{a}}^{2} = {(\overrightarrow{b} + \overrightarrow{c})}^{2}$$.
...
5．已知数列$$\left\{ a_{n} \right\}$$为等比数列，公比为2，且$$a_{1} + a_{2} = 3$$.若$$a_{k} + a_{k + 1} + a_{k + 2} + \cdots + a_{k + 9} = 2^{14} - 2^{4}$$，则正整数$$k$$的值是（    ）
...
【分析】根据题意求出等比数列的通项，在运用等比数列的性质及前*n*项和公式求解即可.

【详解】因为数列$$\left\{ a_{n} \right\}$$为等比数列，公比为2，且$$a_{1} + a_{2} = 3$$，所以$$a_{1} + 2a_{1} = 3$$，解得$$a_{1} = 1$$，

故$$a_{n} = 2^{n - 1}$$，因为$$a_{k} + a_{k + 1} + a_{k + 2} + \cdots + a_{k + 9} = a_{k}(1 + 2 + 2^{2} + ... + 2^{9})= 2^{k - 1} \cdot \frac{1 - 2^{10}}{1 - 2} = 2^{k + 9} - 2^{k - 1} = 2^{14} - 2^{4}$$，解得$$k = 5$$，

故选：B.

6．在锐角$$\bigtriangleup ABC$$中，角$$A,B,C$$所对的边分别为$$a,b,c$$.若$$b - 2c = a\text{cos}C - 2a\text{cos}B$$，则$$\frac{c}{b} =$$（    ）
...
【详解】如图所示，过点*A*作$$AD\bot BC$$于点*D*，

![@@@00ccb0c5-a905-475e-b76a-619d36002861](/Users/muryor/code/mynote/word_to_tex/output/figures/nanjing_yancheng_2025_mock1/media/image2.png){width="1.9583333333333333in"
height="1.15625in"}
...
7．已知双曲线$$C:\frac{x^{2}}{a^{2}} - \frac{y^{2}}{b^{2}} = 1(a > 0,b > 0)$$的左焦点、右顶点分别为$$F,A$$，过点$$F$$倾斜角为$$\frac{\text{π}}{6}$$的直线交$$C$$的两条渐近线分别于点$$M,N$$.若$$\bigtriangleup AMN$$为等边三角形，则双曲线$$C$$的渐近线方程是（    ）

> A．$$y = \pm \frac{\sqrt{3}}{3}x$$ B．$$y = \pm \frac{2\sqrt{3}}{3}x$$
>
> C．$$y = \pm \sqrt{3}x$$ D．$$y = \pm \frac{4\sqrt{3}}{3}x$$

【答案】C

【难度】0.4

【知识点】求直线交点坐标、根据a,b,c齐次式关系求渐近线方程

【分析】求出直线$$MN$$的方程，与渐近线方程联立，求出$$M,N$$的坐标，利用$$\bigtriangleup AMN$$为等边三角形即$$|AM| = |AN|$$，得到$$a,b$$的关系，即可得渐近线方程.

【详解】由题意可得$$A(a,0),F( - c,0)$$，所以直线$$MN$$的方程为$$y = \frac{\sqrt{3}}{3}(x + c)$$，
...
8．已知函数$$f(x) = \frac{1}{a^{2x} + 1} - ax^{3},a > 1$$，则关于$$x$$的不等式$$f\left( x^{2} \right) + f(5x - 6) > 1$$的解集是（    ）

> A．$$( - 6,1)$$ B．$$(2,3)$$ C．$$( - \infty,1)$$ D．$$(2, + \infty)$$

【答案】A

【难度】0.65

【知识点】判断或证明函数的对称性、根据函数的单调性解不等式

【分析】由函数解析式，可得函数的单调性与对称性，化简不等式，可得答案.

【详解】由$$1 - f( - x) = 1 - \frac{1}{a^{- 2x} + 1} + a( - x)^{3} = 1 - \frac{a^{2x}}{1 + a^{2x}} - ax^{3} = \frac{1}{a^{2x} + 1} - ax^{3} = f(x)$$，
...
9．已知$$\text{cos}\alpha\text{cos}\beta = \frac{1}{4},\text{cos}(\alpha + \beta) = \frac{1}{3}$$，则（    ）

> A．$$\text{sin}\alpha\text{sin}\beta = \frac{1}{12}$$
> B．$$\text{cos}(\alpha - \beta) = \frac{1}{6}$$
>
> C．$$\text{tan}\alpha\text{tan}\beta = - \frac{1}{3}$$
> D．$$\text{sin}2\alpha\text{sin}2\beta = \frac{1}{12}$$
...
【详解】由$$\cos(\alpha + \beta) = \cos\alpha\cos\beta - \sin\alpha\sin\beta = \frac{1}{3}$$，且$$\cos\alpha\cos\beta = \frac{1}{4}$$，则$$\sin\alpha\sin\beta = - \frac{1}{12}$$，故A错误；
...
10．在边长为2的菱形$$ABCD$$中，$$\angle BAD = \frac{\text{π}}{3}$$，将菱形$$ABCD$$沿对角线$$BD$$折成四面体$$A'BCD$$，使得$$\angle A'BC = \frac{\text{π}}{2}$$，则（    ）

> A．直线$$A'C$$与直线$$BD$$所成角为$$\frac{\text{π}}{2}$$
>
> B．直线$$A'C$$与平面$$BCD$$所成角的余弦值为$$\frac{\sqrt{6}}{3}$$
>
> C．四面体$$A'BCD$$的体积为$$\frac{4\sqrt{2}}{3}$$
>
> D．四面体$$A'BCD$$外接球的表面积为$$8\text{π}$$

【答案】ABD

【难度】0.65

【知识点】锥体体积的有关计算、多面体与球体内切外接问题、求异面直线所成的角、求线面角

【分析】A.证明$$BD\bot$$平面$$AEC$$即可；B找出线面角$$\angle A'CE$$，在$$\bigtriangleup A'EC$$中求解；C
.
因$$BD\bot$$平面$$AEC$$，则$$V_{三棱锥A' - BCD} = \frac{1}{3}S_{\bigtriangleup A'EC} \cdot |BD|$$；D作垂线，找球心，在$$\bigtriangleup OO_{2}C$$中求解.

【详解】如图所示，取$$BD$$的中点，连接$$AE、CE$$，
...
![@@@641dad21-bc9e-4de6-ba62-8be0539e51f3](/Users/muryor/code/mynote/word_to_tex/output/figures/nanjing_yancheng_2025_mock1/media/image3.png){width="2.84375in"
height="1.5104166666666667in"}

故选：ABD

11．已知函数$$f(x)$$满足：对任意$$x,y \in R,xf(y) + yf(x) = f(xy)$$，且当$$0 < x < 1$$时，$$f(x) > 0$$.下列说法正确的是（    ）
...
【知识点】求函数值、定义法判断或证明函数的单调性、函数奇偶性的定义与判断、用导数判断或证明已知函数的单调性

【分析】由关系$$xf(y) + yf(x) = f(xy)$$取$$x = 0$$，$$y = 0$$可求$$f(0)$$，取$$x = 1$$，$$y = 1$$可求$$f(1)$$，再求$$f(0) + f(1)$$，判断A，取$$y = - 1$$，可得$$f(x)$$，$$f( - x)$$的关系，再将$$x$$替换为$$- 1$$，求$$f( - 1)$$，由此判断函数的奇偶性，判断B，将$$xf(y) + yf(x) = f(xy)$$中的$$y$$用$$\frac{1}{x}$$替换可得$$xf(x) = - x^{3}f\left( \frac{1}{x} \right)$$，结合条件证明当$$x > 1$$时，$$xf(x) < 0$$，再结合函数的奇偶性判断C，结合单调性定义证明函数$$\frac{f(x)}{x}$$在$$(1, + \infty)$$上单调递减，再利用导数证明函数$$y = x \cdot \frac{f(x)}{x}$$在$$(1, + \infty)$$上单调递减，判断D.

【详解】因为$$x,y \in R,xf(y) + yf(x) = f(xy)$$，
...
12．若函数$$f(x) = \text{sin}x + a\text{cos}x$$的图象关于直线$$x = \frac{\text{π}}{6}$$对称，则实数$$a$$的值是
.
...
【知识点】结合三角函数的图象变换求三角函数的性质、用和、差角的余弦公式化简、求值、用和、差角的正弦公式化简、求值

【分析】由条件可得$$f\left( \frac{\text{π}}{6} + x \right) = f\left( \frac{\text{π}}{6} - x \right)$$，化简求$$a$$.

【详解】因为函数$$f(x) = \text{sin}x + a\text{cos}x$$的图象关于直线$$x = \frac{\text{π}}{6}$$对称，
...
所以$$\cos\frac{\text{π}}{6} = a\sin\frac{\text{π}}{6}$$，所以$$a = \sqrt{3}$$.

故答案为：$$\sqrt{3}$$.

13．已知椭圆$$C:\frac{x^{2}}{2} + y^{2} = 1$$的上顶点为$$A$$，直线$$l:y = kx + m$$交$$C$$于$$M,N$$两点.若$$\bigtriangleup AMN$$的重心为$$\left( \frac{1}{2},0 \right)$$，则实数$$k$$的值是
.
...
【知识点】根据直线与椭圆的位置关系求参数或范围、根据韦达定理求参数

【分析】根据椭圆方程求出上顶点$$A$$的坐标，再设出$$M(x_{1},y_{1})$$，$$N(x_{2},y_{2})$$，利用三角形重心坐标公式得到$$x_{1} + x_{2}$$与$$y_{1} + y_{2}$$的值，然后联立直线与椭圆方程，根据韦达定理求出$$x_{1} + x_{2}$$与$$y_{1} + y_{2}$$关于$$k$$、$$m$$的表达式，进而求出$$k$$的值.

【详解】已知椭圆$$C:\frac{x^{2}}{2} + y^{2} = 1$$，上顶点坐标为$$A(0,1)$$.
...
$$m = - \frac{17}{16}$$，此时直线$$l:y = \frac{3}{4}x - \frac{17}{16}$$与椭圆$$\frac{x^{2}}{2} + y^{2} = 1$$有两个交点，符合题意.

故答案为：$$\frac{3}{4}$$.

![@@@db1055da-a643-4a33-a8b3-8ca8869ad810](/Users/muryor/code/mynote/word_to_tex/output/figures/nanjing_yancheng_2025_mock1/media/image4.png){width="1.8125in"
height="1.6041666666666667in"}

14．将9个互不相同的向量$${\overrightarrow{a}}_{i} = \left( x_{i},y_{i} \right),x_{i},y_{i} \in \left\{ - 1,0,1 \right\},i = 1,2,\cdots,9$$，填入$$3 \times 3$$的方格中，使得每行、每列的三个向量的和都相等，则不同的填法种数是
.
...
【知识点】分步乘法计数原理及简单应用、排列数的计算

【分析】先确定每行、每列向量和的可能情况，再根据向量和的情况确定满足条件的向量组合，最后计算不同的填法种数.结合分步乘法原理计算即可.

【详解】已知$${\overrightarrow{a}}_{i} = \left( x_{i},y_{i} \right),x_{i},y_{i} \in \left\{ - 1,0,1 \right\},i = 1,2,\cdots,9$$，
...
所以不同的填法种数是$$36 \times 2 = 72$$种.

故答案为：72.

# 四、解答题

15．如图，$$\bigtriangleup ABC$$内接于圆$$O,AB$$为圆$$O$$的直径，$$CD\bot$$平面$$ABC,E$$为线段$$AD$$中点.

![@@@48b14986-6821-46d4-8bde-f41f17610731](/Users/muryor/code/mynote/word_to_tex/output/figures/nanjing_yancheng_2025_mock1/media/image5.png){width="1.7291666666666667in"
height="1.5833333333333333in"}

(1)求证：平面$$BCE\bot$$平面$$ACD$$；

(2)若$$AB = \sqrt{5},BC = 1,CD = 2\sqrt{3}$$，求平面$$BCE$$与平面$$BDE$$所成锐二面角的余弦值.
...
【分析】（1）易证$$AC\bot BC$$，$$CD\bot BC$$，故$$BC\bot$$平面$$ACD$$，由面面垂直的判定定理即可证明平面$$BCE\bot$$平面$$ACD$$；

（2）建立空间直角坐标系，分别求出平面$$BCE$$与平面$$BDE$$的法向量，利用空间向量即可求空间角.

【详解】（1）因为$$\bigtriangleup ABC$$内接于圆$$O,AB$$为圆$$O$$的直径，所以$$AC\bot BC$$.
...
![@@@67c3292c-9131-4298-8aa9-bdc37fbd3987](/Users/muryor/code/mynote/word_to_tex/output/figures/nanjing_yancheng_2025_mock1/media/image6.png){width="1.8541666666666667in"
height="1.625in"}
...
16．已知数列$$\left\{ a_{n} \right\}$$的前$$n$$项和$$S_{n}$$满足$$\frac{S_{n}}{n} = a_{n} + (1 - n)t,n \in N^{\text{*}},t$$为常数，且$$a_{2} = a_{1} + 2$$.

(1)求$$t$$的值；

(2)证明：$$\left\{ a_{n} \right\}$$为等差数列；

(3)若$$n^{2} < S_{n} < {(n + 1)}^{2},n \in N^{\text{*}}$$，求$$a_{1}$$的取值范围.
...
【知识点】由递推关系证明数列是等差数列、求等差数列前n项和、利用an与sn关系求通项或项、数列不等式恒成立问题

【分析】（1）由关系式$$\frac{S_{n}}{n} = a_{n} + (1 - n)t$$取$$n = 2$$可得$$\frac{S_{2}}{2} = a_{2} - t$$，结合$$S_{2} = a_{1} + a_{2}$$及条件$$a_{2} = a_{1} + 2$$求$$t$$；

（2）将关系式$$\frac{S_{n}}{n} = a_{n} + (1 - n)t$$中的$$n$$用$$n + 1$$替换，与原式相减，结合$$S_{n}$$与$$a_{n}$$关系可得递推式，再证明结论；

（3）由（2）求$$S_{n}$$，代入不等式可得$$1 < a_{1} < 3 + \frac{1}{n}$$恒成立，由此可求$$a_{1}$$的取值范围.

【详解】（1）因为$$\frac{S_{n}}{n} = a_{n} + (1 - n)t,n \in N^{\text{*}}$$，
...
17．甲、乙两人组队准备参加一项挑战比赛，该挑战比赛共分$$n\left( n \in N^{\text{*}},n \geq 2 \right)$$关，规则如下：首先某队员先上场从第一关开始挑战，若挑战成功，则该队员继续挑战下一关，否则该队员被淘汰，并由第二名队员接力，从上一名队员失败的关卡开始继续挑战，当两名队员均被淘汰或者$$n$$关都挑战成功，挑战比赛结束.若甲每一关挑战成功的概率均为$$p(0 < p < 1)$$，乙每一关挑战成功的概率均为$$q(0 < q < 1)$$，且甲、乙两人每关挑战成功与否互不影响，每关成功与否也互不影响.
...
(2)如果$$n$$关都挑战成功，那么比赛挑战成功.试判断甲先出场与乙先出场比赛挑战成功的概率是否相同，并说明理由.

【答案】(1)①$$\frac{1}{3}$$；②$$\frac{19}{18}$$
...
【分析】（1）①记甲先上场且两关都没成功的概率为$$P$$.甲每关不成功概率是$$1 - p$$，乙每关不成功概率是$$1 - q$$.再结合独立事件乘法公式计算即可；

②$$X$$取值有$$0$$，$$1$$，$$2$$.求出对应概率，再按期望算法算出期望.

（2）设甲先出场成功概率为$$P_{1}$$，乙先出场成功概率为$$P_{2}$$.把$$P_{1}$$和$$P_{2}$$展开成式子，对应部分相等，所以$$P_{1} = P_{2}$$，即甲、乙先出场成功概率一样.

【详解】（1）①记甲先上场且挑战没有一关成功的概率为$$P$$，则$$P = (1 - p)(1 - q) = \frac{1}{3}$$.
...
18．已知函数$$f(x) = x\text{e}^{x} + a\text{sin}x$$.

(1)当$$a = 0$$时，求证：$$\frac{f(x)}{x} > x + 1$$；

(2)若$$f(x) > 0$$对于$$x \in \left( 0,\text{π} \right)$$恒成立，求$$a$$的取值范围；

(3)若存在$$x_{1},x_{2} \in \left( 0,\text{π} \right)$$，使得$$f\left( x_{1} \right) = f'\left( x_{2} \right) = 0$$，求证：$$x_{1} < 2x_{2}$$.
...
【知识点】利用导数证明不等式、利用导数研究不等式恒成立问题、利用导数研究函数的零点、导数中的极值偏移问题

【分析】（1）由由$$a = 0$$，得$$f(x) = x\text{e}^{x}$$，构造函数$$g(x) = \text{e}^{x} - x - 1$$，求解单调性，证明结果;

（2）求解$$f'(x) = (x + 1)\text{e}^{x} + a\text{cos}x,$$令$$m(x) = f'(x)$$，则$$m'(x) = (x + 2)\text{e}^{x} - a\text{sin}x$$，分类讨论求解$$a$$的范围;

（3）由（2）知$$a < - 1$$，设$$x_{0} = x_{2}$$，判断$$f(x)$$单调性，$$f\left( x_{1} \right) = 0$$，所以只需证$$f\left( 2x_{2} \right) > 0$$，由$$\left( x_{2} + 1 \right)\text{e}^{x_{2}} + a\text{cos}x_{2} = 0$$，即$$a = - \frac{\left( x_{2} + 1 \right)\text{e}^{x_{2}}}{\text{cos}x_{2}}$$，只需证

$$x_{2}\text{e}^{x_{2}} - \left( x_{2} + 1 \right)\text{sin}x_{2} > 0,x_{2} \in \left( 0,\frac{\text{π}}{2} \right)$$（\*）进而证明结果.

【详解】（1）由$$a = 0$$，得$$f(x) = x\text{e}^{x}$$.
...
$$2^{\circ}$$若$$a < - 1$$，则$$f'(0) < 0,f'\left( \frac{\text{π}}{2} \right) > 0$$，
...
19．设$$M$$是由直线构成的集合，对于曲线$$C$$，若$$C$$上任意一点处的切线均在$$M$$中，且$$M$$中的任意一条直线都是$$C$$上某点处的切线，则称$$C$$为$$M$$的包络曲线.

(1)已知圆$$C_{1}:x^{2} + y^{2} = 1$$为$$M_{1}$$的包络曲线，判断直线$$l:x\text{sin}\theta - y\text{cos}\theta = 1$$（$$\theta$$为常数，$$\theta \in R$$）与集合$$M_{1}$$的关系；

(2)已知$$M_{2}$$的包络曲线为$$C_{2}:x^{2} = 4y$$，直线$$l_{1},l_{2} \in M_{2}$$.设$$l_{1},l_{2}$$与$$C_{2}$$的公共点分别为$$P,Q$$，记$$l_{1} \cap l_{2} = A,C_{2}$$的焦点为$$F$$.
...
【知识点】求在曲线上一点处的切线方程（斜率）、判断直线与圆的位置关系、求直线与抛物线相交所得弦的弦长、圆锥曲线新定义

【分析】（1）求出圆心到直线的距离，即可判断；

（2）解法一：①设$$A(s,t),P\left( x_{1},y_{1} \right),Q\left( x_{2},y_{2} \right)$$，利用导数的几何意义求出切线方程，即可得到直线$$PQ$$的方程为$$y = \frac{1}{2}sx - t$$，再联立直线与抛物线方程，消元，列出韦达定理，根据焦半径公式计算可得；②由①可得$$\frac{|FA|^{2}}{|FP|^{2}} = \frac{|FQ|}{|FP|}$$，求出$$\frac{|FQ|}{|FP|} + \frac{|FP|}{|FQ|}$$的范围，即可求出$$\frac{|FA|}{|FP|}$$的最大值；解法二：①设$$P\left( 2m,m^{2} \right),Q\left( 2n,n^{2} \right),m \neq n$$，利用导数的几何意义求出切线方程，再求出$$A$$点坐标，最后根据弦长公式计算可得；②由①知$$\frac{|FA|^{2}}{|FP|^{2}} = \frac{n^{2} + 1}{m^{2} + 1}$$，设$$t = \frac{n^{2} + 1}{m^{2} + 1}$$，求出$$t + \frac{1}{t}$$的范围，即可求出$$t$$的范围，从而得解.

【详解】（1）圆心$$C_{1}(0,0)$$到$$l$$的距离$$d = \frac{1}{\sqrt{\text{sin}^{2}\theta + {( - \text{cos}\theta)}^{2}}} = 1$$，
...
![@@@93d60e30-fde8-46ad-907a-0ba39f9b0e90](/Users/muryor/code/mynote/word_to_tex/output/figures/nanjing_yancheng_2025_mock1/media/image7.png){width="1.8541666666666667in"
