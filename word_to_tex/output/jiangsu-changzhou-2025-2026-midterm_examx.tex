\examxtitle{江苏省常州市2025-2026学年高三上学期11月期中质量调研数学试题}

\section{单选题}

\begin{question}
已知集合\(A = \left\{ x \mid x^{2} - 5x + 4 < 0 \right\}B = \left\{ n\left| \frac{2n}{n - 1} \right.\  \right.\(是质数,\left. \ n \in \text{N} \right\},则A \cap B =\)(    )\)
\begin{choices}
  \item \(\varnothing\)
  \item {2}
  \item {3}
...
解得\(1 < x < 4\)，
故\(A = \left\{ x\left| 1 < x < 4 \right.\  \right\}\).\par
...
\begin{question}
某店日盈利\(y\)(单位：百元)与当天平均气温\(x\)(单位：\({}^{\circ}\text{C}\))之间有如下数据：
...
小明对上述数据进行分析，发现\(y与x\)之间具有线性相关关系，则\(y关于x\)的经验回归方程为(    )
\begin{choices}
...
\answer{D}
\explain{对于A，
取\(x = 2,x + \frac{1}{x} = 2 + \frac{1}{2} = \frac{5}{2} > 2\)，
A是真命题；\par
对于B，
取\(x = 0,\frac{4}{x + 1} = 4 > 2 = 2 - x\)，
B是真命题；\par
对于C，
...
\begin{question}
已知随机变量\(X \sim N(\mu,9)\)，
若\(P(X < 1 - a) = P(X > 7 + a)\left( a \in \text{R} \right)\)，
则(    )
\begin{choices}
  \item \(E(X) = 3,D(X) = 3\)
  \item \(E(X) = 4,D(X) = 3\)
...
\begin{question}
将函数\(f(x) = \text{cos}(x + \frac{\pi}{3})\)的图象向左平移\(\frac{\pi}{4}\)个单位长度，
再将得到的曲线上每一个点的横坐标变为原来的2倍(纵坐标不变)，
得到函数\(y = g(x)\)的图象，则\(g(x) =\)(    )
\begin{choices}
  \item \(\text{cos}(\frac{x}{2} + \frac{7\pi}{12})\)
  \item \(\text{cos}(\frac{x}{2} + \frac{\pi}{12})\)
  \item \(\text{cos}(2x + \frac{7\pi}{12})\)
  \item \(\text{cos}(2x + \frac{\pi}{12})\)
\end{choices}
\topics{求图象变化前(后)的解析式}
\difficulty{0.85}
\answer{A}
\explain{将函数\(f(x) = \text{cos}(x + \frac{\pi}{3})\)的图象向左平移\(\frac{\pi}{4}\)个单位长度，
得到\(y = \text{cos}(x + \frac{\pi}{4} + \frac{\pi}{3}) = \cos(x + \frac{7\pi}{12})\)，\par
再将得到的曲线上每一个点的横坐标变为原来的2倍(纵坐标不变)，
得\(g(x) = \text{cos}(\frac{x}{2} + \frac{7\pi}{12})\)}
\end{question}
...
\begin{question}
若实数\(x = 10,y = 5\text{eln}2\(，\)z = 2\text{eln}5则x,y,z\)的大小关系是(    )
\begin{choices}
  \item \(x > y > z\)
  \item \(x > z > y\)
  \item \(y > x > z\)
...
\difficulty{0.65}
\answer{A}
\explain{因为\(y = 5\text{eln}2 = \text{eln}2^{5} = \text{eln32},z = 2\text{eln}5 = \text{eln}5^{2} = \text{eln}25\)，\par
由\(\ln 32 > \ln 25 > 0\)，
所以\(\text{e}\ln 32 > \text{e}\ln 25\)，即\(y > z\).\par
设函数\(f(x) = \frac{\ln x}{x}，x > 0\)，
则\(f'(x) = \frac{1 - \ln x}{x^{2}}， x > 0.\par
...
\begin{question}
已知函数\(f(x) = x^{3} + 3ax^{2} + x\left( a \in \text{R} \right),P是函数f(x)\)的图象上的定点，
...
所以\(\{\begin{array}{r} x_{1} + x_{2} - 2x_{0} = 0 \\
2(x_{0}^{3} + 3ax_{0}^{2} + x_{0}) = 2
\end{array}，\par
由\(x_{1} + x_{2} - 2x_{0} = 0及x_{1} + x_{2} = - x_{0} - 3a可得( - x_{0} - 3a) - 2x_{0} = 0\)，解得\(x_{0} = - a\)，\par
由\(2(x_{0}^{3} + 3ax_{0}^{2} + x_{0}) = 2可得x_{0}^{3} + 3ax_{0}^{2} + x_{0} = 1\)，\par
...
\begin{question}
已知在矩形\(ABCD\)中，
\(AB = \sqrt{2},BC = 1,P为线段CD\)的中点，
将\(\bigtriangleup ADP, \bigtriangleup BCP分别沿AP,BP\)翻折，
使得\(C,D\)两点重合于点\(Q\)，则(    )
\begin{choices}
  \item \(AQ\bot BQ\)
  \item 三棱锥\(P - ABQ\)的体积为\(\frac{\sqrt{2}}{4}\)
  \item 点\(Q到平面ABP\)的距离为\(\frac{1}{2}\)
  \item 存在半径为\(\frac{\sqrt{10}}{2}的球O\)，使得\(A,B,P,Q\)四点均在球\(O\)的球面上
\end{choices}
...
又\(AQ,BQ \subset平面ABQ,AQ \cap BQ = Q\)，
故\(PQ\bot平面ABQ\)，\par
...
由\(AQ\bot BQ,PQ\bot平面ABQ\)，取\(AB中点E\)，\par
则\(OE//PQ\)，且\(OP = OQ = r\)，
...
从\(4\)个因式中，
\(3\)个因式选择\(x,1\)个因式选择常数相乘的积即可得含\(x^{3}\)的项，\par
所以含\(x^{3}\)的项的系数为\(- 1 + 2 - 3 + 4 = 2\(.\)2\)}
\end{question}

\begin{question}
已知平面向量\(\overrightarrow{a} = (\text{cos}\alpha,\sqrt{3}\text{sin}(\pi + \alpha)),\overrightarrow{b} = (\sqrt{3}\text{cos}\beta,\text{cos(}\frac{\pi}{2} - \beta))\)，
...
\explain{向量\(\overrightarrow{a} = (\text{cos}\alpha,\sqrt{3}\text{sin}(\pi + \alpha)),\overrightarrow{b} = (\sqrt{3}\text{cos}\beta,\text{cos(}\frac{\pi}{2} - \beta))\)，
由\(\overrightarrow{a}\bot\overrightarrow{b}\)，\par
得\(\overrightarrow{a} \cdot \overrightarrow{b} = \sqrt{3}\text{cos}\alpha\text{cos}\beta + \sqrt{3}\text{sin}(\pi + \alpha)\text{cos(}\frac{\pi}{2} - \beta) = \sqrt{3}\text{(cos}\alpha\text{cos}\beta - \sin\alpha\sin\beta)= \sqrt{3}\cos(\alpha + \beta) = 0\)，
由\(\alpha,\beta\)是锐角，
...
使得\(AD,BC\)重合，得到新的长方形，
发现长边与短边的长度比保持不变．若将长方形纸\(ABCD的顶点A折到边CD\)上，
...
% CONTEXT_AFTER: 情形3： 如图3.当\(F\)与\(O\)重合时，\(PQ\)取得最小值. 如图4，此时\(\begin{tikzpicture}[scale=1.05,>=Stealth,line cap=round,line join=round]
...
\explain{设\(AD中点为E\(,\)CD,AB\)中点依次记为\(M,N\(,\)MN\)中点记为\(O\),连接\(OE\).\par
设\(AB = a,AD = b\),因为\(A'在DC\)上，
所以\(AA'\)的中点在线段\(OE\)上.\par
折痕为\(PQ过F\),且\(PQ\bot AA'\),情况分三种，如图1，2，
3.\par
由题意，
...
所以不同情形种数为\(\text{C}_{4}^{1}\text{C}_{6}^{1}\text{A}_{4}^{4} = 576\).\par
(2)记事件\(A\)：最后1只不合格品正好在第5次测试时被发现，
事件\(B\)：第2次测得合格品，\par
\(P\text{(}A\text{)} = \frac{576}{10 \times 9 \times 8 \times 7 \times 6} = \frac{2}{105}，\)
P\text{(}AB\text{)} = \frac{\text{C}_{6}^{1}\text{A}_{4}^{4}}{10 \times 9 \times 8 \times 7 \times 6} = \frac{1}{210}，\par
...
\(a,b,c\)分别是角\(A,B,C\)所对的边，点\(D在边BC\)上，
已知\(\sin A = \cos\frac{B + C}{2},c = 6, \bigtriangleup ABC\)的面积为\(30\sqrt{3}\)．
\begin{enumerate}[label=(\arabic*)]
  \item 求\(A,a\)；
...
(2)\(- 10\sqrt{3}\)}
\explain{(1)因为\(\text{sin}A = \text{cos}\frac{B + C}{2}\)，
所以\(2\text{sin}\frac{A}{2}\text{cos}\frac{A}{2} = \text{cos}\frac{\pi - A}{2} = \text{sin}\frac{A}{2}\)，\par
因为\(0 < \frac{A}{2} < \frac{\pi}{2}\)，
则\(\text{sin}\frac{A}{2} \neq 0\)，
故由\(\text{cos}\frac{A}{2} = \frac{1}{2}\)，
可得\(A = \frac{2\pi}{3}\).\par
因为\(S_{\bigtriangleup ABC} = \frac{1}{2}bc\sin A = 30\sqrt{3},c = 6\)，
解得\(b = 20\)，\par
由余弦定理得\(a^{2} = b^{2} + c^{2} - 2bc\text{cos}A = 400 + 36 - 2 \times 20 \times 6 \times \left( - \frac{1}{2} \right) = 556\)，\par
解得\(a = 2\sqrt{139}\).\par
(2)设\(\angle BAD = \theta\)，
所以\(\angle CAD = \frac{2\pi}{3} - \theta,0 < \theta < \frac{2\pi}{3}\)．\par
在\(\bigtriangleup ABD\)中，由正弦定理得，
\(\frac{AB}{\text{sin}\angle ADB} = \frac{BD}{\text{sin}\theta}\)，
即\(BD\text{sin}\angle ADB = 6\text{sin}\theta\)，\par
在\(\bigtriangleup ACD\)中，由正弦定理得，
\(\frac{AC}{\text{sin}\angle ADC} = \frac{CD}{\text{sin}(\frac{2\pi}{3} - \theta)}\)，
即\(CD\text{sin}\angle ADC = 20\text{sin}(\frac{2\pi}{3} - \theta)\)，\par
因\(2CD = 3BD,\text{sin}\angle ADB = \text{sin}\angle ADC\)，
代入化简得\(20\text{sin}(\frac{2\pi}{3} - \theta) = 9\sin\theta\)，\par
即\(10\sqrt{3}\cos\theta + 10\sin\theta = 9\sin\theta\)，
解得\(\text{tan}\theta = - 10\sqrt{3}\)，
即\(\text{tan}\angle BAD = - 10\sqrt{3}\).}
\end{question}
...
又\(MN\bot AC_{1},AC_{1} \cap B_{1}C_{1} = C_{1}\)，
\(AC_{1},B_{1}C_{1} \subset平面AB_{1}C_{1}\)，
则\(MN\bot平面AB_{1}C_{1}\)；\par
...
设平面\(MAB_{1}和平面AB_{1}N\)的法向量分别为\(\overrightarrow{m} = \left( x_{1},y_{1},z_{1} \right),\overrightarrow{n} = \left( x_{2},y_{2},z_{2} \right)\)，\par
则\(\left\{ \begin{array}{r}\)
...
(2)\(f(x) = \ln(x + 1) + \cos x\)，
定义域为\(( - 1, + \infty)， f'(x) = \frac{1}{x + 1} - \sin x，\par
①\(x \in ( - 1,0\rbrack\)，
...
由零点存在性定理可得，
存在\(x_{1} \in \left( \frac{1}{\text{e}} - 1,0 \right)使得f\left( x_{1} \right) = 0\)．\par
②当\(x \in \left( 0,\frac{\pi}{2} \right\rbrack,\cos x \geq 0且\ln(x + 1) > 0\)，\par
