...
\mcq[D]
{已知集合 $M=\{\,x\mid x^2-2x-3\le 0\,\}$，$N=\{\,x\mid y=\ln(x-4)\,\}$，则 $(\mathbb{R}\setminus M)\cap N$ 等于（\quad）。}
{$(-1,4]$}
{$(-1,3]$}
{$(3,+\infty)$}
{$(4,+\infty)$}
...
详解：$M=\{\,x\mid x^2-2x-3\le 0\,\}=\{\,x\mid -1\le x\le 3\,\}$，所以 $\mathbb{R}\setminus M=\{\,x\mid x<-1\text{ 或 }x>3\,\}$；又 $N=\{\,x\mid y=\ln(x-4)\,\}=\{\,x\mid x>4\,\}$，所以 $(\mathbb{R}\setminus M)\cap N=\{\,x\mid x>4\,\}=(4,+\infty)$。故选：D。}
...
详解：因为 $AB=\lambda DC$，$|AB|=\sqrt{1+2}=\sqrt3$，$|AD|=\sqrt{16+8}=2\sqrt6$，所以四边形 $ABCD$ 为梯形；又 $AB\cdot AD=1\times4+(-\sqrt2)\times2\sqrt2=0$，所以 $AB\perp AD$，即 $AB\perp AD$，所以四边形 $ABCD$ 为直角梯形，则 $S_{\triangle ABD}=\dfrac12\sqrt3\times2\sqrt6=3\sqrt2$，又四边形 $ABCD$ 的面积为 $12\sqrt2$，所以 $S_{\triangle BCD}=12\sqrt2-3\sqrt2=9\sqrt2$，所以 $\dfrac12\times|CD|\times|AD|=\dfrac12\times\dfrac{\sqrt3}{\lambda}\times|AD|=\dfrac12\times\dfrac{\sqrt3}{\lambda}\times2\sqrt6=9\sqrt2$，所以 $\dfrac1\lambda=\dfrac13$，故选：C。}

\mcq[D]
{已知函数 $f(x)=\dfrac1x-\dfrac1{1-x}+x+k$ 的三个零点为 $a,b,c$，且 $a<b<c$，则下列结论不正确的是（\quad）。}
{$f(x)$ 在 $(0,1)$ 上单调递减}
{曲线 $y=f(x)$ 是中心对称图形}
{$\forall k\in\mathbb{R}$，都有 $c>b+1$}
{$\forall k\in\mathbb{R}$，都有 $c>a+3$}
...
详解：对 A：由 $f'(x)=-\dfrac1{x^2}-\dfrac1{(1-x)^2}-1$，则当 $x\in(-\infty,0)\cup(0,1)\cup(1,+\infty)$ 时，$f'(x)<0$ 恒成立，故 $f(x)$ 在 $(-\infty,0)$、$(0,1)$、$(1,+\infty)$ 上单调递减，故 A 正确；对 B：$f(x)+f(-x+1)=\dfrac1x+\dfrac1{1-x}-x+k+\dfrac1{-x+1}+x-1+k=2k-1$，又 $f(x)$ 定义域为 $(-\infty,0)\cup(0,1)\cup(1,+\infty)$ 故曲线 $y=f(x)$ 关于点 $\left(\dfrac12,k-\dfrac12\right)$ 对称，即曲线 $y=f(x)$ 是中心对称图形，故 B 正确；对 C：由 $f(x)$ 在 $(-\infty,0)$、$(0,1)$、$(1,+\infty)$ 上单调递减，且函数 $f(x)$ 有三个零点，则 $a\in(-\infty,0)$，$b\in(0,1)$，$c\in(1,+\infty)$，则 $b+1\in(1,2)$，有 $f(b+1)=\dfrac1{b+1}+\dfrac1b-b-1+k$，又 $f(b)=\dfrac1b-\dfrac1{1-b}-b+k=0$，即 $k=b+\dfrac1{1-b}-\dfrac1b$，则 $f(b+1)=\dfrac1{b+1}-\dfrac1b-1+b+\dfrac1{1-b}-1+\dfrac1{1-b}-\dfrac2{1-b^2}-1$，由于 $b\in(0,1)$，则 $1-b^2\in(0,1)$，则 $f(b+1)=\dfrac2{1-b^2}-1>1$，又 $c\in(1,+\infty)$，且 $f(c)=0$，$f(x)$ 在 $(1,+\infty)$ 上单调递减，故 $c>b+1$ 对 $\forall k\in\mathbb{R}$ 恒成立，故 C 正确；对 D：取 $k=\dfrac12$，则 $f(x)=\dfrac1x-\dfrac1{1-x}+x+\dfrac12$，则有 $f(-1)=\dfrac1{-1}-\dfrac1{1-(-1)}+\dfrac12(-1)+\dfrac12=0$，$f(2)=\dfrac12-\dfrac1{-1}-2+\dfrac12=0$，即 $a=-1，c=2$，此时 $c=a+3$，故 D 错误。故选：D。}
...
\difficulty{0.65}
\explain{设在 $D$ 处截住足球，时间设为 $t$ 秒，则 $BD=2.5t,\ CD=7.5t$，则 $AD=20-7.5t$；又 $AB=4\sqrt3,\ \angle BAC=30^\circ$，在 $\triangle ABD$ 中，利用余弦定理可知，$BD^2=AD^2+AB^2-2AD\cdot AB\cdot\sin\angle BAC$，则 $(2.5t)^2=(20-7.5t)^2+48-2\times4\sqrt3\times(20-7.5t)\times\dfrac{\sqrt3}{2}$，化简得 $25t^2-110t+104=0$，解得 $t=\dfrac85$ 或 $t=\dfrac{13}5$，所以该球员按原来的速度最快截住足球所用的时间为 $\dfrac85$ 秒。}
\end{question}
...
\difficulty{0.4}
\explain{若 $k=\dfrac2\pi,\ b=0$，求导，利用导数判断 $f(x)$ 的单调性和最值；若 $f'(x)\ge0$ 恒成立，即 $\sin x\ge kx+b$，结合图象可得 $b\le0$，$\pi k+2b-2\le0$；令 $-k^2+\pi k+b=t$，消去 $b$ 结合 $a$ 判别式运算求解即可。\\
