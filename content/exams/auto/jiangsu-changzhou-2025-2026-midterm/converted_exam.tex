...
解得\(1 < x < 4\)，
故\(A = \left\{ x\left| 1 < x < 4 \right.\  \right.\right\}\).\par
...
记圆柱的表面积为\(S_{1}\)，
则\(S_{1} = 2\pi r^{2} + 2\pi rl\)，\par
记圆锥的表面积为\(S_{2}\)，
则\(S_{2} = \pi r^{2} + \pi rl\)，\par
所以圆柱与圆锥表面积之比\(S_{1}:S_{2} = 2:1\)}
\end{question}

\begin{question}
若实数\(x = 10\)，\(y = 5\mathrm{e}\ln2\)，z = 2\mathrm{e}\ln5\)，则\(x,y,z\)的大小关系是(    )
\begin{choices}
  \item \(x > y > z\)
  \item \(x > z > y\)
  \item \(y > x > z\)
...
\difficulty{0.65}
\answer{A}
\explain{因为\(y = 5\mathrm{e}\ln2 = \mathrm{e}\ln2^{5} = \text{eln32},z = 2\mathrm{e}\ln5 = \mathrm{e}\ln5^{2} = \mathrm{e}\ln25\)，\par
由\(\ln 32 > \ln 25 > 0\)，
...
所以\(\text{e}f(2) < 1\)\(\Rightarrow\)\(\frac{\text{e}\ln 2}{2} < 1\)\(\Rightarrow\)\(\text{e}\ln 2 < 2\)\(\Rightarrow\)\(5\text{e}\ln 2 < 10\)，
即\(y < x\).\par
综上，\(x > y > z\)}
\end{question}
...
设直线\(MN\)方程为：\(y - \left( x_{0}^{3} + 3ax_{0}^{2} + x_{0} \right) = k\left( x - x_{0} \right)\)，\(\Rightarrow\)\(y = k\left( x - x_{0} \right) + \left( x_{0}^{3} + 3ax_{0}^{2} + x_{0} \right)\)，\par
...
由\(y_{1} + y_{2} = 2\)\(\Rightarrow\)\(k\left( x_{1} - x_{0} \right) + \left( x_{0}^{3} + 3ax_{0}^{2} + x_{0} \right) + k\left( x_{2} - x_{0} \right) + \left( x_{0}^{3} + 3ax_{0}^{2} + x_{0} \right) = 2\)，\par
即\(k\left( x_{1} + x_{2} - 2x_{0} \right) + 2\left( x_{0}^{3} + 3ax_{0}^{2} + x_{0} \right) = 2\)恒成立.\par
所以\(\left\{\begin{array}{r} x_{1} + x_{2} - 2x_{0} = 0 \\
...
\answer{AC}
\explain{由\(z\)是虚数，\(|z| = 1\)，
设\(z = \cos\theta + \mathrm{i}\sin\theta,\theta \neq k\pi,k \in \text{Z}\)，\par
则\(\frac{1}{z} + z = \frac{1}{\cos\theta + \mathrm{i}\sin\theta} + \cos\theta + \mathrm{i}\sin\theta = 2\cos\theta \in ( - 2,2)\)，\par
...
折痕为\(PQ\)过\(F\)，且\(PQ\bot AA'\)，情况分三种，如图1，2，3.\par
由题意，
...
如图4，
此时\(PQ = \frac{2ON}{\sin\angle ONQ} = \frac{b}{\frac{\sqrt{2}}{\sqrt{3}}} = \frac{\sqrt{6}b}{2}\).\par
...
\begin{enumerate}[label=(\arabic*)]
  \item 当\(a = 1\)，
求\(f(x)\)在点\(\left( 0,f(0) \right)\)处的切线方程；
