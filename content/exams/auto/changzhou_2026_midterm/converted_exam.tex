\setexamdir{content/exams/auto/changzhou_2026_midterm}
...
\begin{question}
已知集合\(A = \left\{ x \mid x^{2} - 5x + 4 < 0 \right\}\)，\(B = \left\{ n \mid \frac{2n}{n - 1} \text{是质数}, n \in \mathbb{N} \right\}\)，
则\(A \cap B =\)(    )
\begin{choices}
  \item \(\varnothing\)
  \item {2}
...
因为\(\frac{2n}{n - 1} = \frac{2(n - 1) + 2}{n - 1} = 2 + \frac{2}{n - 1}\)（\(n \in \mathbb{N},n \neq 1\)），\par
要使\(\frac{2n}{n - 1}\)是质数，
...
小明对上述数据进行分析，发现\(y\)与\(x\)之间具有线性相关关系，
则\(y\)关于\(x\)的经验回归方程为(    )
\begin{choices}
...
\(P\)为线段\(CD\)的中点，
将\(\triangle ADP, \triangle BCP\)分别沿\(AP,BP\)翻折，
使得\(C,D\)两点重合于点\(Q\)，则(    )
\begin{choices}
  \item \(AQ\bot BQ\)
...
\examimage{images/media/image2.png}{0.40}
...
故\(V_{P - ABQ} = \frac{1}{3} \cdot PQ \cdot S_{\triangle ABQ} = \frac{1}{3} \times \frac{\sqrt{2}}{2} \cdot \frac{1}{2} \times 1 \times 1 = \frac{\sqrt{2}}{12}\)，
故B错误；\par
对C：设点\(Q\)到平面\(ABP\)的距离为\(d\)，
则由\(V_{P - ABQ} = V_{Q - ABP}\)可得：\par
\(\frac{\sqrt{2}}{12} = \frac{1}{3} \cdot d \cdot S_{{}_{\triangle ABP}} = \frac{1}{3} \cdot d \cdot \frac{1}{2} \times \sqrt{2} \times 1 = \frac{\sqrt{2}}{6}d\)，
...
\examimage{images/media/image3.png}{0.40}


\examimage{images/media/image4.png}{0.40}


\examimage{images/media/image5.png}{0.40}


\examimage{images/media/image6.png}{0.40}
...
直到4只不合格品全部测出为止．
\begin{enumerate}[label=(\arabic*)]
  \item 求最后1只不合格品正好在第5次测试时被发现的不同情形种数；已知最后1只不合格品正好在第5次测试时被发现，求第2次测得合格品的概率．
\end{enumerate}
...
\(c = 6, \triangle ABC\)的面积为\(30\sqrt{3}\)．
\begin{enumerate}[label=(\arabic*)]
  \item 求\(A,a\)；若\(2CD = 3BD\)，求\(\angle BAD\)的正切值．
\end{enumerate}
\topics{三角恒等变换的化简问题；正弦定理解三角形；三角形面积公式及其应用；余弦定理解三角形}
\difficulty{0.65}
\answer{(1)\(\frac{2\pi}{3},2\sqrt{139}\)
(2)\(- 10\sqrt{3}\)}
\explain{(1)因为\(\sin A = \cos\frac{B + C}{2}\)，
...
因为\(S_{\triangle ABC} = \frac{1}{2}bc\sin A = 30\sqrt{3}\)，
\(c = 6\)，解得\(b = 20\)，\par
由余弦定理得\(a^{2} = b^{2} + c^{2} - 2bc\cos A = 400 + 36 - 2 \times 20 \times 6 \times \left( - \frac{1}{2} \right) = 556\)，\par
解得\(a = 2\sqrt{139}\).\par
(2)设\(\angle BAD = \theta\)，
...
\examimage{images/media/image7.png}{0.40}

\examimage{images/media/image8.png}{0.40}
...
\begin{enumerate}[label=(\arabic*)]
  \item 当\(a = 1\)，求\(f(x)\)在点\(\left( 0,f(0) \right)\)处的切线方程；
  \item 当\(a = 1\)，求函数\(f(x)\)的零点个数；
  \item 若\(\forall x \geq 0,f(x) \geq 0\)，求整数\(a\)的值．
\end{enumerate}
