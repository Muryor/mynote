...
\begin{question}
设抛物线\(C:y^{2} = 2px(p > 0)\)的焦点为\(F\),点\(A\)在\(C\)上，
过\(A\)作\(C\)的准线的垂线，
...
\begin{center}
% IMAGE_TODO_START id=gaokao_2025_national_2-Q6-img1 path=/Users/muryor/code/mynote/word\\_to\\_tex/output/figures/gaokao\\_2025\\_national\\_2/media/image2.png width=60% inline=false question_index=6 sub_index=1
% CONTEXT_BEFORE: |AB| = x_{A} + {2} = 4 + 1 = 5$$. 
\begin{tikzpicture}[scale=1.05,>=Stealth,line cap=round,line join=round]
  % TODO: AI_AGENT_REPLACE_ME (id=gaokao_2025_national_2-Q6-img1)
\end{tikzpicture}
% IMAGE_TODO_END id=gaokao_2025_national_2-Q6-img
1
\end{center}
...
\begin{center}
% IMAGE_TODO_START id=gaokao_2025_national_2-Q11-img1 path=/Users/muryor/code/mynote/word\\_to\\_tex/output/figures/gaokao\\_2025\\_national\\_2/media/image3.png width=60% inline=false question_index=11 sub_index=1
% CONTEXT_AFTER: 方法二：因为$$\tan\angle MOA_{2} = {a}$$，因为双曲线
\begin{tikzpicture}[scale=1.05,>=Stealth,line cap=round,line join=round]
  % TODO: AI_AGENT_REPLACE_ME (id=gaokao_2025_national_2-Q11-img1)
\end{tikzpicture}
% IMAGE_TODO_END id=gaokao_2025_national_2-Q11-img
1
\end{center}


\begin{center}
% IMAGE_TODO_START id=gaokao_2025_national_2-Q11-img2 path=/Users/muryor/code/mynote/word\\_to\\_tex/output/figures/gaokao\\_2025\\_national\\_2/media/image4.png width=60% inline=false question_index=11 sub_index=1
% CONTEXT_AFTER: 方法三：在$$\bigtriangleup OMA_{2}$$利用余弦定理知，$$\left|
\begin{tikzpicture}[scale=1.05,>=Stealth,line cap=round,line join=round]
  % TODO: AI_AGENT_REPLACE_ME (id=gaokao_2025_national_2-Q11-img2)
\end{tikzpicture}
% IMAGE_TODO_END id=gaokao_2025_national_2-Q11-img
2
\end{center}
...
\begin{center}
% IMAGE_TODO_START id=gaokao_2025_national_2-Q14-img1 path=/Users/muryor/code/mynote/word\\_to\\_tex/output/figures/gaokao\\_2025\\_national\\_2/media/image5.png width=60% inline=false question_index=14 sub_index=1
% CONTEXT_BEFORE: 、球的截面的性质及计算 【分析】根据圆柱与球的性质以及球的体积公式可求出球的半径； 【详解】
% CONTEXT_AFTER: 圆柱的底面半径为$$4$$，设铁球的半径为*r*，且$$r < 4$$， 由
\begin{tikzpicture}[scale=1.05,>=Stealth,line cap=round,line join=round]
  % TODO: AI_AGENT_REPLACE_ME (id=gaokao_2025_national_2-Q14-img1)
\end{tikzpicture}
% IMAGE_TODO_END id=gaokao_2025_national_2-Q14-img
1
\end{center}
...
\begin{question}
已知函数\(f(x) = \cos(2x + \varphi)(0 \leq \varphi < \pi)\)，\(f(0) = \frac{1}{2}\)．
...
所以函数\(g(x)\)的单调递减区间为\(\left\lbrack - \frac{\pi}{12} + k\pi,\frac{5\pi}{12} + k\pi \right\rbrack,k \in \mathbb{Z}\)，\par
函数\(g(x)\)的单调递增区间为\(\left\lbrack \frac{5\pi}{12} + k\pi,\frac{11\pi}{12} + k\pi \right\rbrack,k \in \mathbb{Z}\).}
\end{question}
...
\begin{enumerate}[label=(\arabic*)]
  \item 求\emph{C}的方程；
  \item 过点\((0, - 2)\)的直线\emph{l}交\emph{C}于\(A,B\)两点，\(O\)为坐标原点.若\(\bigtriangleup OAB\)的面积为\(\sqrt{2}\)，求\(|AB|\)．
\end{enumerate}

\begin{center}
% IMAGE_TODO_START id=gaokao_2025_national_2-Q16-img1 path=/Users/muryor/code/mynote/word\\_to\\_tex/output/figures/gaokao\\_2025\\_national\\_2/media/image6.png width=60% inline=false question_index=16 sub_index=1
% CONTEXT_AFTER: 由题设直线$$AB$$的斜率不为0，故设直线$$l:x = t(y + 2)$$，$$A\lef
\begin{tikzpicture}[scale=1.05,>=Stealth,line cap=round,line join=round]
  % TODO: AI_AGENT_REPLACE_ME (id=gaokao_2025_national_2-Q16-img1)
\end{tikzpicture}
% IMAGE_TODO_END id=gaokao_2025_national_2-Q16-img
1
\end{center}
...
\begin{center}
% IMAGE_TODO_START id=gaokao_2025_national_2-Q17-img1 path=/Users/muryor/code/mynote/word\\_to\\_tex/output/figures/gaokao\\_2025\\_national\\_2/media/image7.png width=60% inline=false question_index=17 sub_index=1
% CONTEXT_BEFORE: A'$$，使得面$$EFD'A'$$与面*EFCB*所成的二面角为$$60{^\circ}$$．
% CONTEXT_AFTER: (1)证明：$$A'B//$$平面$$CD'F$$； (2)求面$$BCD'$$与面$$EFD
\begin{tikzpicture}[scale=1.05,>=Stealth,line cap=round,line join=round]
  % TODO: AI_AGENT_REPLACE_ME (id=gaokao_2025_national_2-Q17-img1)
\end{tikzpicture}
% IMAGE_TODO_END id=gaokao_2025_national_2-Q17-img
1
\end{center}


\begin{center}
% IMAGE_TODO_START id=gaokao_2025_national_2-Q17-img2 path=/Users/muryor/code/mynote/word\\_to\\_tex/output/figures/gaokao\\_2025\\_national\\_2/media/image8.png width=60% inline=false question_index=17 sub_index=1
% CONTEXT_BEFORE: \subset$$平面$$A'EB$$，所以$$A'B//$$平面$$CD'F$$． (2)
% CONTEXT_AFTER: 因为$$\angle DAB = 90{^\circ}$$，所以$$AD\bot AB$$，又因
\begin{tikzpicture}[scale=1.05,>=Stealth,line cap=round,line join=round]
  % TODO: AI_AGENT_REPLACE_ME (id=gaokao_2025_national_2-Q17-img2)
\end{tikzpicture}
% IMAGE_TODO_END id=gaokao_2025_national_2-Q17-img
2
\end{center}
