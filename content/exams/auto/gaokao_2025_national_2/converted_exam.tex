...
\examimage{content/exams/auto/gaokao_2025_national_2/images/media/image2.png}{0.30}
...
\begin{question}
记\(S_{n}\)为等差数列\(\left\{ a_{n} \right\}\)的前\(n\)项和.若\(S_{3} = 6,S_{5} = - 5\),则\(S_{6} =\)(   )
\begin{choices}
  \item \(- 20\)
  \item \(- 15\)
  \item \(- 10\)
...
\difficulty{0.85}
\answer{B}
\explain{设等差数列\(\left\{ a_{n} \right\}\)的公差为\(d\)，
则由题可得
\(\left\{ \begin{array}{r}
3a_{1} + 3d = 6 \\
5a_{1} + 10d = - 5
...
\begin{question}
记\(S_{n}\)为等比数列\(\left\{ a_{n} \right\}\)的前\(n\)项和，
...
\begin{question}
已知\(f(x)\)是定义在\(\mathbf{R}\)上的奇函数，且当\(x > 0\)时，
...
左、右顶点分别为\(A_{1},A_{2}\)，
以\(F_{1}F_{2}\)为直径的圆与\(C\)的一条渐近线交于\(M\)，
\(N\)两点，
...
\examimage{content/exams/auto/gaokao_2025_national_2/images/media/image3.png}{0.30}


\examimage{content/exams/auto/gaokao_2025_national_2/images/media/image4.png}{0.30}
...
方法三：在\(\triangle OMA_{2}\)利用余弦定理知，\(\left| MA_{2} \right|^{2} = |OM|^{2} + \left| OA_{2} \right|^{2} - 2|OM|\left| OA_{2} \right|\cos\angle MOA_{2}\)，\par
即\(\left| MA_{2} \right|^{2} = c^{2} + a^{2} - 2ac \cdot \frac{a}{c} = b^{2}\)，则\(\left| MA_{2} \right| = b\)，\par
则\(\triangle A_{1}A_{2}M\)为直角三角形，且\(\angle A_{1}MA_{2} = \frac{\pi}{6}\)，则\(2\left| MA_{2} \right| = \sqrt{3}\left| MA_{1} \right|\)，故B错误；\par
...
故\(b = 2\sqrt{6}\)，故四边形\(NA_{1}MA_{2}\)为\(2S_{\triangle MA_{1}A_{2}} = 2 \times \frac{1}{2} \times 2\sqrt{6} \times 2\sqrt{2} = 8\sqrt{3}\)，\par
故D正确}
\end{question}
...
\examimage{content/exams/auto/gaokao_2025_national_2/images/media/image5.png}{0.30}

\topics{圆柱的结构特征辨析；球的截面的性质及计算}
\difficulty{0.4}
\answer{\(2.5\)}
\explain{圆柱的底面半径为\(4\text{cm}\)，设铁球的半径为\(r\)，
且\(r < 4\)，\par
...
长轴长为4．
\begin{enumerate}[label=(\arabic*)]
  \item 求\(C\)的方程；过点\((0, - 2)\)的直线\(l\)交\(C\)于\(A,B\)两点，
\(O\)为坐标原点.若\(\triangle OAB\)的面积为\(\sqrt{2}\)，
求\(|AB|\)．
\end{enumerate}

\examimage{content/exams/auto/gaokao_2025_national_2/images/media/image6.png}{0.30}
...
故\(S_{\triangle OAB} = \frac{1}{2} \times |2t| \times \left| y_{1} - y_{2} \right| = |t|\sqrt{\left( y_{1} + y_{2} \right)^{2} - 4y_{1}y_{2}} = \frac{|t|\sqrt{32 - 16t^{2}}}{t^{2} + 2} = \sqrt{2}\)，\par
...
\examimage{content/exams/auto/gaokao_2025_national_2/images/media/image7.png}{0.30}


\examimage{content/exams/auto/gaokao_2025_national_2/images/media/image8.png}{0.30}
...
记\(p_{k}\)为打完\(k\)个球后甲比乙至少多得2分的概率，
\(q_{k}\)为打完\(k\)个球后乙比甲至少多得2分的概率．
\begin{enumerate}[label=(\arabic*)]
  \item 求\(p_{3},p_{4}\)(用\(p\)表示)．若\(\frac{p_{4} - p_{3}}{q_{4} - q_{3}} = 4\)，
求\(p\)．
  \item 证明：对任意正整数\(m\)，
